...
| Charging | `/api/charging`, `/api/admin` | ChargeableEntity CRUD, **distribution versions** (effective-dated draft/active, create with blank/copy-active/copy-prior origin, activate with date + rationale, version diff, per-entity Stage 1 view) and **distribution edges** (scoped to a draft version, with per-edge rationale), BTC profiles, rollup query, location breakdowns, WBS preview |
