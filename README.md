...
| Scenarios | `/api/scenarios` | CRUD, actions, comparison, AI advisor, BTC sandbox, 8-dimension impact, anchor + rebase + archive, promote, apply-to-forecast, **project-scope editable forecast grid** (`GET /{id}/projects` all selectable projects; `GET /{id}/projects/{pid}/grid` resolved overlay grid; `PUT`/`DELETE …/cells` cell-overlay write + revert per cell/line/all) |
