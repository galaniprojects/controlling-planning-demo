...
| Scenarios | `/api/scenarios` | CRUD, actions, comparison, AI advisor, BTC sandbox, 8-dimension impact, anchor + rebase + archive, promote, apply-to-forecast, **project-scope editable forecast grid** (`GET /{id}/projects/{pid}/grid` resolved overlay grid; `PUT`/`DELETE …/cells` cell-overlay write + revert per cell/line/all) |
...
A regression test plan lives in [`qa/test-plan.md`](qa/test-plan.md) — 204 scenarios across 15 test suites covering all modules, personas, and features. Backend unit tests run with `python -m pytest tests/ -v` (2216 tests).
