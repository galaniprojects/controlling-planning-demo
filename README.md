...
### What-If Simulator
Scenario planning with a WYSIWYG project-scope editing surface and a portfolio-scope bulk-lever catalogue. Scenarios anchor against a specific forecast version rather than rebasing automatically. Tier 1 levers cover forecast grid edits, pipeline transitions, and milestone slides; Tier 2 covers rate-table and Tech Navigator overrides; Tier 3 covers people / capacity-parameter / restructuring scenarios and cost-allocation edits, all gated by an explicit visibility flag.

**Project scope — direct grid editing.** At project scope the primary surface is the directly-editable forecast grid. Users edit cells, add/remove role lines, add/remove/edit external-cost line items, adjust milestones/DoI/stage, and move project dates in place. A macro strip provides four configurable bulk helpers: Delay by N months, Accelerate by N months, Pause for N months, Remove. Internal-line € values are computed at the RateTable rate **in force at each cell's month** (rate-at-month, not latest-wins) — so a rate step mid-year prices later cells at the new rate, re-entering the same hours leaves € unchanged, and values the grid shows exactly match what Promote will write to the live forecast. **Project Leads can author scenarios scoped to their own projects** (private, owner-visible; PLs cannot promote but can apply scenarios to their own forecast via Apply-to-forecast).

**Portfolio scope — bulk levers** (across-the-board cut, cut-by-LoB, cut-by-type, freeze-new-starts, cap-cost-category, rate-escalation) remain form-driven for managing dozens of projects simultaneously. Tech Navigator profile edits are also available.

**Cost-allocation sandbox (Stage 1 + Stage 2).** The fully-editable cost-allocation surface lets controllers model Stage-1 distribution edge changes (including adding new destinations) and Stage-2 BTC profile line shifts. Edits stay isolated in a per-scenario sandbox (Stage 1 forks into a scenario-scoped `DistributionVersion` discriminated by `scenario_id`, anchored to a pinned production version; BTC line edits live as `ScenarioAction` overlays — canonical rows never mutated) until you explicitly **Promote**, which routes each diff through its native system path. An 8-dimension impact dashboard (financial, backlog ranking, capacity, people, outsourcing ratio, investment mix, running cost, change summary) updates on each edit.

**Apply-to-forecast and Promote** both create draft Change Requests as artefacts of scenario-driven changes, fed back into the normal CR approval workflow rather than bypassing it. **Apply-to-forecast** (PL-scoped) creates one draft CR per cost centre per affected project, grouped by cost centre, so the PL can refine them in the Workbench before submission. **Promote** of other-PL forecast diffs creates draft CRs authored by the affected project's PL (keeping them informed and in control). Cost-allocation Promote writes live edges as before, unchanged. Project Leads use **Apply-to-forecast** to carry their own-project diffs into the next cycle — from their own scenarios or from a published leadership scenario.
...
- **Dynamic present time (living demo):** VIPER's "today" is the **real current month**, not a fixed date — `config.get_current_period()` derives it from the system clock and `GET /api/config` exposes `{current_period, open_forecast_month, fiscal_year}` to the frontend on bootstrap. The in-progress current month is **locked for forecasting** (it carries partial actuals); the first editable forecast month is the next one. The committed canonical seed is anchored at `2026-04` and **date-shifted forward at load / on `/admin/reset-demo`** (`seed/date_shift.py`) so actuals always end at the current month, forecasts always open next month, and every narrative offset is preserved — all time-dependent logic (actuals cutoffs, forecast boundaries, elapsed-month tinting) keys off this anchor.
...
|--------|-------------|---------|
| Config | `/api` | **`GET /config`** — application temporal context `{current_period, open_forecast_month, fiscal_year}` derived from the real date (unauthenticated; read on frontend bootstrap so backend and frontend never disagree on the current period or the editable forecast boundary) |
...
| Scenarios | `/api/scenarios` | CRUD, actions, comparison, AI advisor, cost-allocation sandbox, 8-dimension impact, **per-project anchors + rebase** (each touched project anchored to its own latest forecast cycle; `GET /{id}/rebase-options` feeds the labeled per-project picker, `PUT /{id}/rebase` body `{anchors:{project_id:version_id}}`; the stale-guard refuses only the stale project(s)) + archive, **promote + apply-to-forecast create draft Change Requests** (apply: per cost centre per owned project; promote: per other-PL project for the owning PL), **project-scope editable forecast grid** (`GET /{id}/projects` all selectable projects; `GET /{id}/projects/{pid}/grid` resolved overlay grid; `PUT`/`DELETE …/cells` cell-overlay write + revert per cell/line/all; `POST …/projects/{pid}/lines` add role line / `DELETE …/lines/{line_key}` remove (open to all authors); `GET`/`PUT`/`DELETE …/projects/{pid}/plan` project-plan overlay (start/end dates — date edits shift the resolved window — stage, DoI, milestones); `GET`/`PUT`/`DELETE …/projects/{pid}/mix` **Tier-3** seniority/sourcing mix swap (writes gated on Tier-3); `GET`/`POST`/`PUT`/`DELETE …/projects/{pid}/external-costs[/{line_key}]` external-cost line items add/remove/edit — vendor, category (cost-type), description, capex). **Project Leads author their own project-scoped scenarios** (private, owner-visible, scoped to their owned projects — the demo ships `scn-pl-predmaint-defer`, Priya Sharma deferring Predictive Maintenance PoC 3 months); promote + apply-to-forecast widen to the plan / mix / external-edit overlay kinds with an editable-boundary guard (Simulator S4). **Cost-allocation sandbox routes:** `POST/PUT/DELETE /{id}/cost-allocation/distributions[/{edgeId}]`, `POST /{id}/cost-allocation/to-business`, `POST /{id}/cost-allocation/btc-lines`, `GET /{id}/cost-allocation/cost-allocation-impact?year=` |
