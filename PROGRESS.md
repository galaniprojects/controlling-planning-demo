...
- [x] **Session 1 — Storage foundation, compile-down & macro engine (backend keystone)** (agent team `sim-s1`, 4 teammates in parallel + lead integration, single branch `feature/simulator-project-scope-s1` off `main`, 2026-06-02). Backend only; no UI. **Suite 2169 passed / 0 failed.** Branch tip `2f49545`.
  ...
  - **Decisions / open items:** (1) Demo-story call — dwh descope re-expressed as a believable uniform 30% (−€58.5k) rather than honouring the stale notional −€165k. (2) **Macro-shift-on-promote is an open design question** — a delay macro promotes via its `forecast_grid` action but currently writes only overlay cells, so the curve shift is not yet materialised to live forecast; whether promote should write the full resolved grid for macro projects (vs per-entry overlay, which preserves partial promote) is undecided. (3) Actuals write-guard deferred (demo edits are future-dated; UI makes actuals read-only). (4) Mix is double-represented for `scn-cco-mdh-staffing` (legacy `change_allocation` action + `ScenarioMixChange`); harmless in S1 (core doesn't apply mix to financials yet) — single-source in Session 3. (5) Pre-existing generator drift: `s12_pipeline` doesn't emit `handover_month` for `proj-iam-run` (re-applied by hand on regen) — fix the generator separately.
