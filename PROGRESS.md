...
- [ ] **FD-3 — Stage 1 effective-dated rework** (agent team, branch `feature/fd3-stage1-effective-dated`). Realises spec §4 + tags `[F-S1-01..08]`: first-class `DistributionVersion` header (draft/active, effective-dated, provenance, scenario-scoped fork), per-edge rationale, version diff, per-entity Stage 1 surface, anchor pinning for lever 12. Plan: `~/.claude/plans/sprightly-beaming-quail.md`. In progress.
