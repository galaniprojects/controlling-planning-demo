...
- [x] **FD-1 — Data foundation** (solo session, branch `feature/fd1-um-data-foundation`). Model + service layer only, no authoring UI. Realises spec §2 (UM model, state machine, provenance, audit) + §3 (allocation-key field). Tags `[F-UM-01..05]`, `[F-AK-01]`, partial `[F-DIR-01]`.
  ...
  - **Tests:** new `test_user_measurement_service.py` (25 cases: integer gate field+CSV, draft/active immutability, copy lineage, per-cell audit, resolution) + `test_chargeable_entity_allocation_key.py`; updated `test_btc_service.py` / `test_router_admin_user_measurement.py` / `test_router_btc_profile.py` (version model, repurposed the `.000000` regression into a version-resolution test). **Full suite green: 1775 passed** (proves lever 12 / Stage 1 / rollup untouched).
