...
## Current Status
Phase: v5 Wave 2 — C1 (mixed-granularity forecast + versioning) complete on branch `feature/v5-c1-mixed-granularity`. PR not yet opened (user gates).
Last completed: C1 (mixed-granularity forecast grid + version history backend, +94 tests, 882 total). Unblocks B1 (scenarios anchor to a forecast version) and C2 (frontend mixed-grid + version-history panel).
**Post-merge requirement:** drop `creta_demo.db` and re-seed (`rm backend/creta_demo.db && python main.py && curl -X POST .../api/admin/reset-demo`) — `is_provisional` column on `forecasts` table breaks existing DB until reseed.
