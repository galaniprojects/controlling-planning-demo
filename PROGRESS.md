...
## What-If Simulator — E2E Fixes (active)

Active spec: `guides/Simulator_E2E_Fixes_Spec.md` + `Simulator_E2E_Fixes_Implementation_Guide.md` (3-session wave fixing defects from the browser E2E pass `qa/bug-report-simulator-e2e.md`). Session 1 = frontend stabilization + apply-gate reconciliation (no backend behaviour-contract change, no reseed); Session 2 = promote→draft-CR / apply→draft-CR keystone (done); Session 3 = per-project anchor schema + seed anchors + rebase picker (done — Issue B / F10 / F11; the final session of the wave).

- [x] **Session 1 — Frontend stabilization & apply gate** (agent team `sim-e2e-s1`, 4 teammates with disjoint file ownership + lead integration/visual pass, branch `fix/sim-e2e-s1-stabilization` off `main`, 2026-06-04). Implements spec §5 (apply gate), §7 (recalc lifecycle), §8 (Tier-3 nav), §9 (graceful Stage-2 BTC), §10 (display/formatting). **Frontend vitest 165/165; backend pytest 2370 passed; `tsc --noEmit` clean for all touched files** (pre-existing ~89-error `tsc -b` baseline in unrelated modules unchanged). Visual-verified light + dark @1440px → `qa/screenshots/sim-e2e-s1/`.
  - **F7 (blocker) — Tier-3 Resources sidebar nav:** `ResourcesSection` now self-navigates via `useNavigate`/`useParams` (mirrors the working `PortfolioSettingsSection`), dropping the never-passed `onSelectSurface` prop; all 6 entries reach their surface/action.
  - **F4 (borderline blocker) — stuck Recalculate:** the `SET_IMPACT` reducer case in `ScenarioContext` now clears `loading`/`error` (was only cleared by `SET_DETAIL`/`LOAD_ERROR`), so the control returns to "Recalculate" and the freshness indicator updates without a reload. +reducer tests.
  - **Issue A (§5) — apply gate:** FE `ApplyButton` widened to `disabled={!(isOwner || isPublished)}` with an accurate tooltip; the BE check (`scenario_apply_forecast.py`, already owner-or-published) reconciled + pinned by 3 new `TestApplyGate` cases (owner-private allowed, published-non-owner allowed, private-non-owner rejected). No behaviour-contract change — apply still writes provisional cells (apply→draft-CR is Session 2).
  - **F6 (§9) — graceful Stage-2 BTC:** shared `EntityBTCProfileEditor` now recognizes the expected "No BTC profile" 404 for Internal Services (the detail carries neither "not found" nor "404") and renders an Internal-Service-aware "Automatic — UM-derived" read-only card instead of the "Failed to load BTC profile" hard error; benefits both the live charging module and the sandbox.
  - **F1 (§10) — headline impact:** `formatHeadlineImpact` (`ScenarioRow`) handles all impact shapes — financial-delta (+ budget-neutral schedule-shift variant), BTC pct-shift, staffing/capacity mix — with European-formatted summaries, never raw JSON. +vitest against the real seed payloads.
  - **F5 / F9 (§10) — timestamps:** relative "Recalculated N ago" (`StaleIndicator`) and the promote-complete + promote-history timestamps now parse the backend's naive-UTC strings as UTC via a shared `parseServerTimestamp` (`lib/formatters.ts`) instead of as local time.
  - **Display/obs (§10):** change-summary badge excludes lifecycle events (`changeSummary.diffEntries`); stacked rebase/anchor dialogs no longer stack (`ApplyConfirmModal` gates its outer `Dialog` on `!rebaseOpen`); empty `WideSlideOver` dialog kept out of the a11y tree when closed (mounted-but-stripped: `inert`/`aria-hidden` + conditional `role`, preserving the slide-out animation); leaked spec tags removed from the apply provenance string (immediate) and a seeded scenario description (`scenarios.py` + `seed.sql`; surfaces on next `reset-demo`).
  - **Code-review gate — independent `code-reviewer-fresh` (no blockers; 4 should-fix, all addressed by the lead):** **#2** extracted the shared `parseServerTimestamp` and reused it at both promote-timestamp sites (`PromoteReviewPage` + `PromoteAuditDrawer`) — same UTC-skew class as F5; **#3** reverted `WideSlideOver`'s `return null` (which killed the slide-out animation) to the mounted-but-stripped approach above; **#4** `renderSurface` now passes the `:entityId` segment as `PeopleMasterSurface`'s `initialActionId` so the 4 restructuring shortcuts pre-open their form (not the generic tile list); **#1 (F2) — DEFERRED at the time, NOW CLOSED (see below):** the reviewer flagged the live-local location qualifier as inert. That diagnosis was an artifact of S1 having been branched off a **stale local `main` (#125) that predated the #126 merge** — on that base the simulator grid genuinely carried no workforce-location dimension, so the FE plumbing was inert and was reverted (4 files restored to `main`). On current `main` the S6 location dimension (`location_id`/`location_name` on rows, 4-segment `line_key`, location-aware rates) **is already merged** (#126), so F2 is a cheap ~30-line FE thread, not a feature. The earlier "needs S6 ported + reseed" framing was inaccurate for post-#126 `main`.
  - [x] **F2 (nit) — CLOSED** (branch `fix/sim-e2e-f2-location-labels` off `main` post-#127, 2026-06-04). `computeLineTotals` (`liveLocal.ts`) now carries `location_name` onto each `LineTotal`, and `LiveLocalPanel`'s Internal Resources summary renders it via `LocationLabel` (`kind="workforce"`) exactly like the grid rows — same-role/multi-location lines (e.g. Data Engineer — Munich vs — Budapest) now disambiguate. +vitest assertion; frontend vitest green, `tsc --noEmit` clean.
- [x] **Session 2 — Promote→draft-CR (F8) + Apply→draft-CR keystone** (agent team `sim-e2e-s2`, lead-solo keystone → 4 parallel lanes with disjoint file ownership + lead integration, branch `fix/sim-e2e-s2-promote-apply-cr` off merged `main` (#127), 2026-06-04). Implements spec §2 (shared CR factory), §3 (promote→CR / F8), §4 (apply→draft-CR). **Full backend pytest green (2400+); frontend vitest 76/76 (simulator), `tsc --noEmit` 0 errors.**
  - **Keystone — `services/change_request_factory.py` (NEW):** extracts the Rolling Forecast Review wizard's inline CR creation into one reusable capability. `create_change_requests(...)` builds one CR per `CRGroupInput` (per-group `status` override — wizard keeps `pending_*`, simulator passes `draft`), `CRChangeDetail` rows, and resource requests; `group_details_by_cost_center(...)` mirrors the wizard's CC grouping (`Allocation → Person.cost_center_id`); **re-run dedupe** replaces this scenario's prior **draft** CRs for the project (keyed on `source_scenario_id`, leaving non-draft CRs untouched). The relocated `_create_resource_requests_from_cr` is **parameterized on the cost centre** — fixes a latent bug where it was hard-coded to `cc-muc-apd` (mis-targeting any role in another CC). `diff_grids_to_details(adjusted, anchor)` (in `routing.py`) emits the per-cell change list (internal → hours, external → €, matching the wizard's RR math). `submit_forecast_cycle` rewired to the factory (behaviour-preserving). +`tests/test_change_request_factory.py` (9 tests).
  - **Schema — `ChangeRequest.source_scenario_id`** (nullable Integer, **logical** reference to `scenarios.id` — a hard FK would close a `change_requests → scenarios → forecast_versions → change_requests` cycle SQLite can't order for create/drop without ALTER). Provenance + dedupe; NULL for wizard CRs. **Dev DB must be rebuilt** (`create_all` won't add a column to an existing table) — delete `viper_demo.db` then `/admin/reset-demo`.
  - **F8 (blocker) — Promote → draft CR** (`scenario_promote.py`): the macro and overlay other-PL branches now create one `draft` CR per cost centre via the factory, **authored by the project's PL** (`_project_pl_id`), tagged `source_scenario_id`; status flips `skipped → promoted`; empty net diff → no CR. The coarse per-action `apply_routing` `change_request` branch is a clarified no-op message (creation centralised in the grid-aware loops to avoid double-creation). Cost-allocation promote untouched (still writes the live edge).
  - **Apply → draft CR** (`scenario_apply_forecast.py`): `_materialize_project` now diffs the resolved grid vs the live anchor and stages it as one `draft` CR per cost centre (authored by the applying PL), **replacing** the `is_provisional` cell write. The `is_provisional` **column stays** (separate `[C-FG-07]` granularity-boundary use). Apply gate + stale-anchor guard unchanged, still firing before creation. Audit event keeps its column names but `diffs_carried_forward`/`diffs_skipped` now mean projects-with-CR / skipped-or-empty; response gains `draft_change_requests_created`. Empty-diff project → skipped (correctly handles project-scope no-op actions like `reduce_budget`).
  - **Frontend** (`scenariosApi.ts`, `ApplyConfirmModal`, `PromoteReviewPage`, `PromoteAuditDrawer`): result UIs show "N draft change requests created" and deep-link each to its project's Workbench Change History (`navigateToWorkbenchByProject`). New optional response fields are backward compatible.
  - **Lead integration fixes:** reconciled the `ApplyToForecastResponse` Pydantic schema (`schemas/scenarios.py`) — added `draft_change_requests_created` + per-item `change_requests_created`, and made `action_id` Optional (overlay-only items carry `action_id=None`, which would have 500'd through the response_model). Updated the router integration test `test_router_scenarios_b1.py` (the only apply test going through the response_model) to use a real grid-changing overlay edit.
  - **Docs:** `docs/data-model.md` (new column + apply/promote behaviour on `ScenarioApplyToForecastEvent`), in-app manual `whatif_simulator.json`, README. The wave spec already prescribes this behaviour.
  - **Visual verification** (1440px, dev DB rebuilt for the new column → `qa/screenshots/sim-e2e-s2/`): apply-to-forecast end-to-end — confirm modal shows the draft-CR copy, success modal reports "Draft change requests created: 3" with per-project breakdown + provenance; DB confirms 3 `draft` CRs on proj-predmaint authored by the PL with `source_scenario_id` set; re-apply dedupe holds at 3 (not 6). The F11 NULL-anchor 409 + rebase flow rendered correctly throughout (Session-3 territory).
  - **Code-review gate — independent `code-reviewer-fresh` (no blockers; 3 should-fix + 1 nit actioned, 2 nits noted):** **SF1/SF2** — empty-diff promote routes (resolved diff nets to zero) were still reported as created CRs (inflating the count + rendering dead Workbench deep-links) and inflated `promoted_count`; now they report `status="skipped"` with `change_requests_created=0`, the real per-row CR count + `change_request_id`/`project_id` are threaded onto the promote summary (and `PromoteResultItem` schema), and the FE (`PromoteReviewPage`, `PromoteAuditDrawer`) keys off `change_requests_created > 0` instead of `routing_type`. **SF3** — a project with no allocations yielded a synthetic `"unknown"` cost-centre id that would write a dangling `ResourceRequest` (SQLite FK enforcement off); `create_resource_requests_from_cr` now falls back to a real CC for any unresolvable id (+test). **Nit 6** — aligned the `data-model.md` FK-cycle wording with the model comment. Noted (not actioned): nit 4 (re-derived PL id), nit 5 (diffs can include locked/past months).
  - **Remaining future work:** Session 3 (per-project anchor schema, seed anchors, rebase picker — Issue B / F10 / F11) — done, see below.
- [x] **Session 3 — Per-project anchor schema + seed anchors + rebase picker** (lead-solo keystone → 2 parallel sub-agent lanes (`react-specialist` FE picker ∥ `test-writer` tests) + lead docs/integration, branch `fix/sim-e2e-s3-per-project-anchor` off `main` (#128), 2026-06-05). Implements spec §6 (Issue B, F10, F11). **Model change → delete-DB + reseed (no Alembic); doc trifecta carried.**
  - **Schema — `ScenarioProjectAnchor` (`scenario_project_anchors`, NEW):** one row per project a scenario touches (`scenario_id`, `project_id`, `forecast_version_id`, `rebased_from_version_id`; `UniqueConstraint(scenario_id, project_id)`; `ix_scenario_project_anchors_scenario`). **Replaces** the scalar `Scenario.anchor_forecast_version_id` / `rebased_from_version_id` columns + their relationships, which are **removed** (full replacement, no mirror). `Scenario.project_anchors` relationship (cascade all, delete-orphan).
  - **Why (the bug):** the former stale-guard found "latest cycle" globally via `version_type=='cycle'` ordered `created_at DESC` `.first()`; all seeded cycle versions share one timestamp, so the tie-break was non-deterministic (picked the lowest id, `proj-mdh-rollout`), 409-ing apply/promote on every other project (Issue B). Per-project anchoring resolves "latest" by **max `version_number` per project** — deterministic under tied timestamps.
  - **Keystone — `services/scenario_anchor.py` (NEW):** single home for anchor logic, reused by create/promote/apply/impact/rebase — `latest_cycle_version_for_project`, `touched_project_ids`, `ensure_anchors_for_touched` (idempotent; pins a touched project to its latest cycle the first time it is seen, never re-pins an existing/rebased row), `stale_projects` (per-project guard; returns only the behind projects; no-cycle tolerance preserved), `rebase_anchors` (validates each version is a `cycle` belonging to the project, records `rebased_from_version_id`, upserts), `anchor_version_ids`, `anchor_total` (sum of per-project anchor totals). `AnchorError`.
  - **Guards migrated:** `scenario_promote.assert_anchor_is_latest_cycle` and `scenario_apply_forecast.assert_anchor_is_latest_cycle` now delegate to `stale_projects` and 409 naming **only** the stale project(s). Anchors are pinned at the create (clone) path and at the action-add / overlay-edit chokepoints (alongside `modified_at`) so interactively-authored scenarios anchor each project as it is first touched.
  - **Seed anchors (F11):** `loader._seed_scenario_anchors()` runs after `_seed_forecast_versions()` (both load + reset paths) and calls `ensure_anchors_for_touched` per seeded scenario — same helper as runtime, so seed and live stay consistent. The dead "set on first open" NULL columns were dropped from `s19_scenarios.py` and the committed `seed.sql` scenarios INSERT. **Verified live:** reseed pins 6 anchors across 4 scenarios — scn 4 `proj-predmaint` (the F11 case) and multi-project scn 2 (3 anchors) now anchored to each project's `v2 (Q2 2026 Cycle)`; apply no longer 409s.
  - **Rebase picker (F10):** new `GET /api/scenarios/{id}/rebase-options` returns, per touched project, its current anchor + `is_stale` + labeled cycle candidates; `PUT /api/scenarios/{id}/rebase` body is now `{anchors: {project_id: version_id}}`. FE `RebaseModal` rewritten as a per-project batch picker (one labeled `v{n} — {cycle_label}` `Select` per project, stale/current badges, mirrors `workbench/forecast/VersionSelector`); `scenariosApi`/`ScenarioManagerPage`/`types/api.ts` updated. **Frontend `tsc --noEmit` 0 errors.**
  - **Schema/response renames:** list/detail/impact/promote-preview now carry `anchor_version_ids` (dict) instead of the scalar; `ScenarioCreate` dropped `anchor_forecast_version_id`. Response-model reconcile done as lead integration (the recurring S2 trap).
  - **Docs:** `docs/data-model.md` (new `ScenarioProjectAnchor` section + `Scenario` updated), CLAUDE.md model index (Scenarios family), in-app manual `whatif_simulator.json` (per-project anchor + labeled picker), this entry.
  - **Tests / verification:** full backend suite **2423 passed / 0 failed** (+17 new `tests/test_scenario_anchor.py`; the 4 anchor-dependent test files rewritten to the per-project model); frontend `tsc --noEmit` 0 errors, vitest 172/172. Endpoints smoke-tested via TestClient against a clean reseed (rebase-options, rebase round-trip v1→stale/v2→current, bad-version 404, access control). **Browser (chrome-devtools MCP, 1440px, light + dark → `qa/screenshots/sim-e2e-s3/`):** as PL the formerly-NULL `proj-predmaint` scenario applies with **no 409** (F11); the Rebase modal shows the **labeled per-project picker** (`v2 — Q2 2026 Cycle`, not a raw integer) for both single-project (predmaint) and multi-project (Budget Pressure: 3 projects) scenarios, with both labeled candidates in the dropdown (F10). Per-project stale-guard confirmed (rebase to v1 → only that project stale, v2 → current).
  - **Behaviour note (intentional):** the per-project guard iterates *active* `ScenarioProjectAnchor` rows (project still touched) only — a project that gains its *first* cycle after being touched (so it carries no anchor row) is not refused. Does not occur in the demo (all projects ship 2 cycles, so every touched project is anchored at first touch); documented in the service docstrings.
  - **Code-review gate — independent `code-reviewer-fresh` (2 blockers + 4 should-fix + 4 nits; all actioned):** the review caught that the FE contract migration was **incomplete** — only the 3 files the FE lane edited were updated. Crucially it surfaced that the "tsc clean" signal was bogus: the frontend root `tsconfig.json` is solution-style (`"files": []` + references), so `tsc --noEmit` is a **no-op**; the real gate is **`tsc -b`** (baseline ~90 errors in unrelated modules). Fixes (solo): **B1** `ApplyConfirmModal` rebase-recovery wiring (passed the removed `currentAnchorVersionId`, missing `scenarioId`, old `onConfirm` signature) — broke the apply-to-forecast 409→rebase path; **B2** `ScenarioContext.rebase` still posted `{new_anchor_version_id}`; **SF3-5** stale `anchor_forecast_version_id` reads in `ScenarioContext`/`CompareSelectionPage`/`ComparePage` + the FE type decls (`types/api.ts`, `impactTypes.ts`, `scenariosApi.ts`) → migrated to `anchor_version_ids` (compare gate now does per-project anchor-map equality); **SF6** orphan anchors — `scenario_anchor._active_anchors` filters `stale_projects`/`anchor_version_ids`/`anchor_total` to currently-touched projects so a project the scenario stopped touching can't 409 promote/apply; **nit7** `rebase_anchors` rejects untouched projects; `rebase_options` lists touched only; **nit8** hoisted local imports in promote/apply; **nit9** `_seed_scenario_anchors` now fails loudly; **nit10** in-app `DataModelTab` Scenario fields. +2 new tests (orphan-anchor ignored; rebase membership) and fixture touches so anchored projects reflect reality.
  - **Re-verification:** `tsc -b` back to the **90-error baseline, 0 in touched files** (the 2 new compile errors gone); vitest 172/172; full backend suite green; clean reseed (6 anchors / 4 scenarios) via the loud-failure path; `/admin/reset-demo` re-anchors correctly. **Browser (chrome-devtools MCP, 1440px → `qa/screenshots/sim-e2e-s3/`):** the previously-broken path now works end-to-end — Apply-to-Forecast on a stale scenario fires the 409 guard, the "Anchor is out of date" dialog → Rebase… opens the per-project picker (Stale badge, v1→v2) *from within* ApplyConfirmModal, and re-Apply then succeeds ("3 draft change requests created").
  - **Second code-review pass — independent `code-reviewer-fresh` (no blockers; 1 should-fix + nits; the recommended subset actioned):** **SF1** — the orphan-anchor filter (`_active_anchors`) was applied to the impact dashboard but *not* to three other readers (`_serialize_scenario` list endpoint, `get_scenario_detail` metadata, `compare_scenarios` shared-anchor gate), which still built the anchor map from raw `scenario.project_anchors` — so a scenario carrying an orphan row could show a list/detail map disagreeing with its dashboard and be wrongly 409'd from Compare. All three now route through `anchor_version_ids(db, s)` (the serializer gained a `db` param). **Nit2** — `rebase_anchors` now skips the write (no spurious self-rebase audit row) when the target version equals the current anchor (+test). Nits 3 (read/write permission split) and 4 (`sys.path` insert) reviewed and intentionally left. tsc -b unchanged (90/0-in-touched); affected backend tests green.

- [x] **Fix — NEW-SIM-NOOP: 7 silent no-op simulator levers + NEW-1..4 FE polish** (agent team `sim-noop-fix`, 3 implementers — `engine` ∥ `tests` ∥ `frontend` — with disjoint file ownership + lead integration/verification, branch `fix/sim-noop-levers` off `origin/main` (#129), 2026-06-05). Closes the round-2 regression finding (`qa/bug-report-simulator-e2e.md` → "Round 2"): 7 levers were accepted (HTTP 200), recorded as `ScenarioAction`s, shown in the Change Summary, but produced **zero impact** — the engine had no dispatch branch (six of them) or the FE surface and engine disagreed on param names (`rate_escalation`). Same defect class as the original F8 silent-drop. **All Impact dimensions derive from the engine's `recalculate_scenario` working state, so a lever "works" iff its branch mutates `adjusted_budget` (or, for re-bucket, the node a project aggregates under).** No model/schema change (working-state fields are in-memory only → no `docs/data-model.md`/CLAUDE.md index edit). **49 backend unit tests green; isolated curl harness ALL PASS; browser spot-check confirmed.**
  - **A0 — working state + helpers** (`services/scenario_engine.py`): `recalculate_scenario` now carries `transformation_level` + `project_type` per project; two reused helpers — `_uplift_forecast_by_category(db, working, project_ids|None, category, pct, from_month, sub_categories=None)` (sums matching `Forecast` per project, adds `sum*pct/100`) and `_resolve_top_level_node(db, node_id, top_type)` (walks `GroupingEntity.parent_entity_id` to the top-level ancestor, mirroring `portfolio_service`).
  - **A1 `cut_by_hierarchy`** — resolve node → member projects via `_get_projects_for_entity_recursive`, cut by `percentage` (year-scope aware, mirrors `cut_by_type`). **A2 `cut_by_transformation`** — filter working by `transformation_level`, cut by `percentage`. **A4 `change_budget_envelope`** {mode,value,year} — percent: `± value%` on that-year forecast; absolute: **pro-rata scale-all** (`ratio = value / current_year_total`) so the year envelope sums to the target. **A5 `inject_hypothetical_project`** — inserts a synthetic working entry keyed `hypo-{slug(name)}` (collision-suffixed, deterministic on recalc replay), `adjusted_budget=total_budget` so it flows into Financial/Backlog/opex. **A6 `reassign_hierarchy`** (project-scope, **re-bucket only**) — sets `state["reassigned_node_id"]`, **no budget change**; threaded onto `project_states`; `compute_investment_mix_dimension` (`services/scenario_impact.py`) now buckets `anchor_total` by the canonical node but `scenario_total` by the reassigned node (origin node down, target node up). **A7 `rate_escalation`** — `_apply_rate_escalation` keeps the existing per-person path when `scope_type` is present, and adds a forecast-category uplift path for the two surface contracts (`{pct, rate_scope}` and `{pct, from_month, category, hierarchy_node_id?}`).
  - **A3 `adjust_rate_table`** {rate_table_scope, location_id?, role_type_id?, percentage, effective_month} — **scoping corrected mid-session** (engine flagged a contract-vs-test divergence; lead verified the data model and ruled): for `category='internal'` forecast rows the **role_type_id lives in `Forecast.sub_category`** (`models/financial.py:29,40`; `report_builder_catalog.py:90/105`), so `role_type_id` scopes precisely on forecast rows via `sub_categories=[role]` — **not** via allocations (which over-approximates). `location_id` (no forecast-row location signal) derives the project set via `CostCenter.location_id → Person.cost_center_id → allocated projects`. Contract corrected in `qa/CONTRACTS-sim-noop.md`; engine + tests re-aligned.
  - **NEW-1/NEW-2 (FE)** `HierarchyReassignSurface.tsx` — raw `ge-…` text field replaced with a depth-indented active-hierarchy `Select` (local `flattenHierarchy` over `useActiveHierarchy().entityTree`, not the shared `ActionForm`); when reached with no project segment, a project `Select` (sourced from `detail.project_states`) appears so the action always carries a real `project_id` + `hierarchy_node_id` (Reassign disabled until both set). **NEW-3 (FE)** `BacklogSandboxSurface.tsx` — mounted the shared `BacklogFilterBar` (stage / project-type / size / FY / T-level + cutoff toggle) wired to the `SandboxBacklogProvider` filter state. **NEW-4 (FE)** `ApplyConfirmModal.tsx` — "Carry diffs from this **published/private** scenario…" copy now keys off `detail.metadata.status` instead of hard-coding "published".
  - **Verification (Part D):** isolated `TestClient` harness (`qa/verify_sim_noop.py`, live config → `current_period=2026-06`) replays the round-2 lead method — control `across_the_board_cut` then each lever, asserting the cumulative `financial.total_delta` moves: **6 cost levers all move** (cut_by_hierarchy −650K, cut_by_transformation −237K, adjust_rate_table +147K / role +16K, envelope percent −325K / absolute +1.75M, inject +750K exact, rate_escalation +121K / +179K); **reassign_hierarchy leaves financial flat and re-buckets investment-mix** (he-dnd→he-tbs). Backend suite `test_scenario_engine.py` + `test_scenario_impact.py` = **49 passed**. **Browser (Playwright, 1440px, Controller → `qa/screenshots/`):** scenario Impact strip shows **Financial +€766K ▲ +7,8%** (matches harness) with all dimensions populated; Hierarchy Reassign node + project dropdowns and the populated node list; Backlog filter bar. `/admin/reset-demo` at the end.
  - **Known limitation (flagged):** `inject_hypothetical_project` has no `Forecast` rows, so the synthetic project is absent from the per-year `time_frame_breakdown` (built from `yearly_fc` before actions apply) — the Overall/Financial total is correct; documented in a code comment. **Out-of-scope finding (logged, not fixed):** `cut_by_type` has the same param-mismatch class — `catalogueDef.ts` sends `target_type` = `'1'|'2'|'non_type_3'` but the engine reads `'all'|'service'|'project'`, so the catalogue entry never matches (the lead's curl control used backend-native `target_type:"all"`); needs sign-off before bundling.
  - **Code-review follow-up — independent `code-reviewer-fresh` (no blockers; 2 should-fix + 2 nits, all 4 actioned by user request):** **R1 (should-fix)** — `reassign_hierarchy` was silently dropped for "core-handled" projects (those with a Layer-2 cell edit or macro): the action loop's `elif core_handled:` / `impact_delta_json` core paths skip `_apply_action`, so `reassigned_node_id` never got set → the exact accepted-but-no-op class. Added `_CORE_PASSTHROUGH_ACTIONS = {"reassign_hierarchy"}` and a top-of-loop passthrough applying these non-financial re-bucket actions even when core (no budget effect, can't disturb the grid recompute). **R2 (should-fix)** — `adjust_rate_table` with `rate_table_scope="external"` + a `role_type_id` filtered on `Forecast.sub_category`, but role only lives there for internal rows (external rows carry `cost_type_id`) → silent near-no-op; the role sub_category filter now applies for internal scope only (external+role uplifts all in-scope external forecast). **R3 (nit)** — `_uplift_forecast_by_category` set `is_affected=True` unconditionally even on a 0-scope uplift; now gated inside `if scoped:` like the `cut_*` branches. **R4 (nit)** — a non-top-level/orphan reassign node (resolver fallback) could split anchor vs scenario in the mix dimension; the reassign branch now stores `reassigned_node_id` only when resolution lands on a genuine top-level node (chosen over restricting the FE picker — sub-node picks already resolve to top-level correctly, so the picker isn't the defect; NEW-1 UI unchanged). **+4 unit tests** (engine: external+role, zero-scope is_affected, orphan-node skip; impact: reassign-on-core re-bucket). **Full backend suite 2448 passed**; isolation harness `qa/verify_sim_noop.py` still ALL PASS.

## What-If Simulator — Project-Scope Redesign (active)

Active spec: `guides/Simulator_Project_Scope_Redesign_Spec.md` + 6-session implementation guide. Rebuilds the project-scope path of the What-If Simulator around a two-layer storage model + a clean recompute core (replacing the aggregate-budget heuristics). One session per sitting; human checkpoint after Session 2.

- [x] **Session 1 — Storage foundation, compile-down & macro engine (backend keystone)** (agent team `sim-s1`, 4 teammates in parallel + lead integration, single branch `feature/simulator-project-scope-s1` off `main`, 2026-06-02). Backend only; no UI. **Suite 2169 passed / 0 failed.** Branch tip `2f49545`.
  - **Phase 0 — frozen contract (`3793297`):** two-layer storage model. Layer 1 (macros) reuses `ScenarioAction` ordered transforms — no new table. Layer 2 (hand overlay) = **4 new typed tables** `ScenarioForecastCellEdit` (hot path, cells) / `ScenarioLineEdit` (add/remove role + external lines) / `ScenarioMixChange` (Tier-3 mix) / `ScenarioPlanEdit` (dates/stage/DoI/milestone), keyed by (scenario, project, line, month). Vocab constants `OVERLAY_*`, `PROJECT_MACRO_ACTION_TYPES`. Frozen interface seam in `services/scenario_project_scope/types.py` (`ResolvedGrid/Cell/Line`, `RoutableDiff`, `ProjectStateDict`, `GridSplits`, `MacroApplyResult`). **Anchor cell source = live `Forecast` rows** (per-month, write-path keyspace); `anchor_forecast_version_id` stays scalar ref + stale guard. Models added to CLAUDE.md index (66→70) + `docs/data-model.md`.
  - **Recompute core — resolution + macros + rollup (`d89ed20`):** `resolve_project_grid` = anchor → ordered macros → Layer-2 overlay (hand-edits-win, absolute-month keyed). Macros (delay/accelerate/pause/remove) are curve transforms preserving shape + total; accelerate clamps at the current open forecast month (`>= DEMO_DATE`, per spec §4's worked example) with a user note; delay(+N)∘accelerate(−N) is identity. Rollup reproduces the existing `project_states` shape (RAG via shared `compute_*` helpers) so the impact dashboard is insulated; adds internal/external + seniority splits.
  - **Direct overlay routing + de-stubbed write paths (`e381a07`):** overlay rows route DIRECTLY per-entry via `collect_overlay_diffs` → `RoutableDiff` (no synthetic-action round-trip; partial promote preserved). Promote's `direct_forecast_update` is a real writer of live `Forecast` rows (hours→€ via effective rate; minted/removed lines). Apply-to-forecast materialises resolved values as provisional cells **with values** (was flag-only). Promote orchestrator now routes forecast overlay for projects with no `forecast_grid` action (own→direct write, other→change request) so overlay-only project edits promote.
  - **Engine integration (`fe7b3fc`):** `recalculate_scenario` routes new-model projects (macro action or cell/line overlay) through resolve→rollup; legacy non-macro project actions survive as the compile target for non-overlay projects; portfolio + lever-12 untouched. Caught + fixed a cross-stream bug: the macro N-reader didn't recognise the seed's `delay_months`/`advance_months` keys, silently making every macro a 1-month shift.
  - **Seed re-expression (`2f49545`):** the 3 scenarios re-expressed. `scn-budget-pressure-15`: delay/accelerate as Layer-1 macros, `reduce_budget(dwh,30%)` → 22 `ScenarioForecastCellEdit` rows (uniform ×0.70, external €195k→€136.5k, −€58.5k); ScenarioState + headline **regenerated from the new engine** (delay/accelerate are total-preserving → budget_delta 0; dwh −€58.5k). `scn-cco-mdh-staffing` mix → `ScenarioMixChange`; `scn-mdh-rebalance` (lever-12) untouched.
  - **Decisions / open items:** (1) Demo-story call — dwh descope re-expressed as a believable uniform 30% (−€58.5k) rather than honouring the stale notional −€165k. (2) Actuals write-guard deferred (demo edits are future-dated; UI makes actuals read-only). (3) Mix is double-represented for `scn-cco-mdh-staffing` (legacy `change_allocation` action + `ScenarioMixChange`); harmless in S1 (core doesn't apply mix to financials yet) — single-source in Session 3. (4) Pre-existing generator drift: `s12_pipeline` doesn't emit `handover_month` for `proj-iam-run` (re-applied by hand on regen) — fix the generator separately.
  - **Code-review follow-ups — ALL FIXED** (`31a7105`, 2026-06-02). Independent `code-reviewer-fresh` returned 1 blocker / 5 should-fix / 4 nits; all addressed, suite **2176** (+7 tests). Shared `rates.py` leaf (effective rate used by resolution + routing) and `routing._write_grid_cells` / `write_grid_replacing_project` extracted. **#1 (blocker):** hours cell edits recompute € via the effective rate in resolution, so the dashboard matches what promote writes. **#2:** `grid_splits.by_role` keys on `role_type_id or sub_category`. **#3:** apply-to-forecast materialises a PL's own overlay-only projects (no action). **#4 (macro-on-promote — decided HYBRID full-grid write):** macros now write their full resolved (shifted) curve to live forecast via `_macro_forecast_routes` + replace semantics (own→direct, other→change request; per-action loop skips macros; overlay route excludes macro projects; replace deletes the months a shift vacated to avoid double-count). **#5:** time-frame breakdown derives per-year € from the resolved grid for core projects (live + snapshot) so a macro's year shift is visible. **#6** headline `action_count` clarified; **#7** dropped unused cycle params; **#8** rate docstring; **#9** data-model.md 66→70; **#10** seed test docstring. `seed.sql` byte-unchanged (the only seed-config touch was a comment).

- [x] **Session 2 — Editable grid: thin vertical slice (first feel)** (contract prelude + 2-lane sub-agent fan-out — `fastapi-developer` ∥ `react-specialist` — + lead integration & visual verification, branch `feature/simulator-project-scope-s2` off **live `main`** @ `aa54f8f`, 2026-06-03). The first frontend slice of the redesign: makes the simulator's embedded forecast grid **writable**, wired to Session 1's overlay, with live-local feedback. **Backend suite 2216 passed / 0 failed** (+16 router tests); frontend **+31 vitest**, `tsc` clean for touched files. **⚠️ Human checkpoint reached — awaiting sign-off on the feel before Session 3.**
  - **Contract prelude (`1633942`):** froze the editable-grid seam before fan-out — purpose-built `ScenarioGrid*` response shapes + `CellEditRequest` (`backend/schemas/scenarios.py`) and the mirrored TS types + five `scenariosApi` wrappers (`getScenarioGrid` / `writeCellOverlay` / `revertCell` / `revertLine` / `clearProjectOverlay`). Pure-monthly columns (drop the workbench mixed near-monthly/far-quarterly model); macros reuse the existing `applyAction` / `removeAction`.
  - **Backend lane (`e741154`):** three endpoints on the scenario router. `GET /{id}/projects/{pid}/grid` → resolved grid via a shared `_grid_to_response` helper (reuses S1 `resolve_project_grid` + `read_anchor_grid` + `effective_hourly_rate`; anchor value per cell; `can_edit=month>=DEMO_DATE`). `PUT …/cells` upserts `ScenarioForecastCellEdit` on its unique key with an actuals-write guard (403 on `month<DEMO_DATE`) + field↔line-kind coherence (422). `DELETE …/cells?line_key&month` reverts per cell / line / clear-all (cell-edit rows only — never line/mix/plan edits or macros). Write/revert tail mirrors `apply_action` (snapshot invalidation + `recalculate_scenario`), returning `{state, grid}` for one-round-trip reconcile. Macros confirmed fully served by existing `/actions`.
  - **Frontend lane (`dc6cfeb`):** rebuilt `ForecastGridSurface` as a thin orchestrator around the editable `MonthCategoryGrid` primitive (replacing the read-only `MixedGranularityGrid`). New `surfaces/forecast-grid/` — pure helpers (`forecastGridAdapter`, `liveLocal`, `quarterMath`), a surface-local `useForecastWorkingEdits` hook (optimistic write → reconcile from the response grid → rollback on failure), `MacroStrip` (delay/pause/accelerate N + remove → `applyAction`; active-macro chips → `removeAction`; clamp note), `LiveLocalPanel` (line totals / budget / delta-vs-anchor move live; **RAG = last server value, "updates on Recalculate"** per the confirmed decision), `RevertControls` (per-cell/line/all). Four additive `ScenarioContext` methods wrap the API + reuse the stale/change-feed dispatch; the working-edits Map stays surface-local. **Pause macro sends `{months:N}`** (the engine's `_get_n` honours it; the old surface's `pause_month` date was a no-op — evidenced in `macros.py`).
  - **Integration fix — `has_overlay` (`47b2d71`):** visual verification caught that a macro-only project (railsafety) listed all 78 macro-shifted cells as per-cell-revertable "Edits" and highlighted them, because both the adapter and `RevertControls` keyed off `is_changed` (adjusted-vs-anchor, which folds in macro shifts) rather than actual overlay presence. Added a `has_overlay` flag to `ScenarioGridCell` (true only when a `ScenarioForecastCellEdit` row exists) and drove the changed-highlight + revert list off it. Now railsafety shows 0 revertable edits (curve just moved); dwh shows exactly its 22 hand-overlays. +2 backend assertions, +1 adapter test.
  - **Visual verification (Playwright, 1440px, light + dark → `qa/screenshots/s2-*.png`):** confirmed all done-when items live — typing 500 into Data Engineer Jul-26 moved budget 384.300→424.300 €, delta −58.500→−18.500 €, line +40.000, RAG reconciled Red→Green, stale banner lit; actuals guard (API: 403 on `<2026-04`); accelerate-12 on dwh surfaced the amber clamp note ("can move only 3 month(s) before … 2026-04"); per-cell revert restored the cell + dropped Edits 23→22; dark-mode parity clean (semantic colours, dark RAG badge, European formatting). Demo DB reset to pristine after testing.
  - **Decisions / open items:** (1) Live RAG = last server value (refreshes on each write's recalc), not a client mirror. (2) Grid columns pure-monthly. (3) **Checkpoint discussion item:** accelerating a project that carries absolute-month cell overlays shifts the curve but leaves the overlays absolute-keyed, so a descope's reductions partially "fall off" the shifted months (per spec §5 absolute-month keying / hand-edits-win) — worth a demo-story look. (4) Actuals-distinct styling is in place but unexercised in the seed (the affected projects start ≥2026-07, no pre-DEMO_DATE columns). (5) `ChangeSummaryKind` not extended (overlay feed entries reuse `'action'`) to avoid an out-of-scope edit; revisit in S3.

- [x] **Session 2 — review fixes + project-selector fix** (solo, same branch, 2026-06-03). Independent `code-reviewer-fresh` returned 1 blocker / 3 should-fix / 4 nits; the user also reported the project selector not showing all projects. All addressed. **Backend suite 2221 passed / 0 failed** (+5 tests); frontend **32 vitest**, `tsc` clean for touched files. Visual-verified light + dark → `qa/screenshots/s2fix-*.png`. Demo DB reset to pristine.
  - **Project-selector bug (root cause + fix, user decision = all active projects):** the simulator sidebar `ProjectsSection` called `portfolioApi.getProjects()` → `/api/portfolio/projects`, hard-coded to `population="change"`, which drops backlog-stage projects — so projects the scenario edits (proj-dwh "Under Evaluation", proj-connveh "Proposed") were unreachable. New scenario-scoped `GET /api/scenarios/{id}/projects` returns **all active projects** (excl. Cancelled; run-entities are not `Project` rows) ordered by name; `ProjectsSection` now fetches it (+ `scenariosApi.getScenarioProjects`). Verified live: the selector now lists all 27 projects incl. Connected Vehicle Platform + Data Warehouse Consolidation.
  - **#1 (blocker) — live-local anchor €:** the grid response carried only anchor *hours* for internal lines, so `liveLocal.ts` reconstructed anchor € as `hours × latest-rate`, disagreeing with the server's stored anchor €. Added `anchor_amount_eur` to `ScenarioGridCell` (the anchor cell's stored €); `liveLocal` now takes anchor + unedited-cell € from the server (`anchor_amount_eur` / `amount_eur`) and only derives edited cells locally (`hours×rate`, matching the server write recompute). Live-verified: dwh panel now reads budget 339.900 € / anchor 398.400 € (server-stored) instead of the rate-inflated 384.300 / 442.800; delta unchanged at −58.500. +rate-mismatch vitest + backend `anchor_amount_eur` assertion.
  - **#2 (should-fix) — CC-Owner scoping (user decision = drop):** removed `cost_center_owner` from `require_role` on the cell write + revert endpoints (the scoped PL/CC access layer is Session 4). +CC-Owner→403 test.
  - **#3 (should-fix) — optimistic-rollback race:** `useForecastWorkingEdits` now captures the rollback baseline **inside** the functional updater and restores only the affected key(s), so the quarterly fan-out's concurrent writes can't drop a sibling pending edit on a failed call (all four paths: commit, revert cell/line/all).
  - **#4 (perf) — drop the double resolve:** the write path validated field/kind off a full `_grid_to_response` resolve; now uses the existing `_line_kind_for_key(line_key)`, leaving a single grid resolve for the response.
  - **Nits:** **#6** `DELETE …/cells` now rejects a `field`/`month` filter without `line_key` (422 — prevents an unintended bulk wipe) +test; **#8** the destructive Remove-project macro now requires a confirm `Dialog` (mirrors "Revert all edits"); **#5** server `is_changed` kept + commented as the macro+overlay drift signal (frontend drives off `has_overlay`); **#7** `effective_hourly_rate` is latest-rate not rate-at-month — noted out of scope (the residual "edit-to-same-hours changes €" oddity on rate-mismatched seed lines; a seed/rate-table consistency concern).
  - **Verification:** full `pytest tests/ -q` → **2221 passed / 0 failed**; `forecast-grid` vitest 32/32; `tsc` clean for touched files (pre-existing ~89-error baseline unchanged). Live: selector lists all projects; Remove-macro confirm dialog renders (light + dark); panel anchor/budget now server-consistent.

- [ ] **Session 3 — Complete the edit surfaces (breadth)** (implementation complete, awaiting code-review gate, branch `feature/simulator-project-scope-s3` off live `main` @ `5ce3783`, 2026-06-03). Implements spec §3 (full edit catalogue + cost-allocation gap closure), §7 (debounced cross-portfolio feedback), §8 (Tier-3 mix). Lead-does-Phase-0-then-team orchestration: agent team `sim-s3` (3 teammates `lines-plan`/`external-costs`/`cost-alloc` on the single shared branch + lead Phase-0 scaffold & integration). **Full backend suite 2279 passed / 0 failed; simulator vitest 53 passed; `tsc` clean.**
  - **Phase 0 — scaffold (lead, structural, behaviour-preserving):** extracted the project-scope HTTP surface out of the 1500-line `routers/scenarios.py` monolith into **`routers/scenarios_project_scope.py`** (same `/api/scenarios` prefix; the 4 Session-2 endpoints + their 5 helpers moved verbatim; orphaned schema/model imports cleaned from `scenarios.py`; registered in `main.py`). This de-conflicts the router layer for parallel work: T1 (role lines / plan / Tier-3 mix) and T2 (external-cost line items) add endpoints under separate banner regions + **separate endpoint families** (`/projects/{pid}/lines` vs `/external-costs`) so handlers never overlap; T3 (cost-allocation gap closure — the long pole) stays isolated in `scenarios.py`'s lever-12 region + `scenario_lever12`/`distribution_service`. Wrote **`guides/Simulator_S3_Team_Conventions.md`** (ownership map, endpoint seam, FE shared-file conventions, model/enum doc protocol, S4 scope guards). Debounced cross-portfolio feedback (§7) + any FE slot refactor deferred to lead integration. **Suite 2221 passed / 0 failed** (unchanged — pure refactor).
  - **T1 — Role lines (add/remove) + plan edits + Tier-3 mix (spec §3 items 3/6/7, §8)** (teammate `lines-plan`). **No model/enum change** — used the existing `ScenarioLineEdit` (add/remove), `ScenarioPlanEdit` (`OVERLAY_PLAN_TARGETS`), and `ScenarioMixChange` shapes as-designed in S1. **Resolution (`services/scenario_project_scope/resolution._apply_overlay`):** added plan + mix application (was cell+line only). New order: line add/remove → **plan date shift** → **Tier-3 mix swap** → per-cell edits (cells stay the last word, §5.1). A `start_month` plan edit shifts the movable curve via the macros' `_shift_grid` primitive so the grid reflects the shifted window (§3 item 7); `end_month` sets the window boundary; stage/DoI/milestone targets are stored + surfaced for the editor + the DoI gate at promote (not applied to the cell grid). Mix moves `hours_per_month_swap` hours/month from the swap-from role line to the swap-to line from `effective_from` onward, recomputing € on both via the effective rate — so the investment-mix / outsourcing splits reflect it (this is the S1 open-item #3 single-source: mix now drives the project-scope financials, not just a promote-time routing). **Backend:** under the T1 banners in `routers/scenarios_project_scope.py` — `POST …/lines` + `DELETE …/lines/{line_key}` (controller/executive + owner; **not** Tier-3 — open to all authors); `GET/PUT/DELETE …/plan` (date/stage/DoI/milestone overlay, past-date guarded); `GET/PUT/DELETE …/mix` (**writes gated on `user_has_tier3`**, past-month guarded). Reuses `_recalc_after_overlay_write`/`_grid_to_response`/`_line_kind_for_key`/`_require_project`. New schemas `LineAdd*`, `Plan*`, `Mix*` in `schemas/scenarios.py`. **Frontend:** net-new `RoleLineControls.tsx`, `PlanEditorModal.tsx`, `Tier3MixControl.tsx` (renders **null** for non-Tier-3 via `useTier3` — not greyed), each mounted as one child of `ForecastGridSurface` (`onMutated→bumpReconcile`); T1 client methods (`scenariosApi.ts`) + context mutations (`ScenarioContext.tsx`, each MARK_STALE + change-feed like `applyCellOverlay`) under `// --- T1: lines/plan/mix ---` banners. **Tests:** +22 backend (`tests/test_scenario_project_scope_t1.py` — 5 resolution unit for plan-shift/mix/cell-wins + 17 router integration incl. the Tier-3 403 write gate) ; +2 frontend vitest (`Tier3MixControl.test.tsx` render-null path); full backend suite 2267 passed; `tsc` clean. **Known limit:** mix/plan resolve in the sandbox + dashboard but promote/apply-to-forecast widening to these overlay kinds is Session 4.
  - **T2 — External-cost line items add / remove / edit (spec §3 item 5)** (teammate `external-costs`). **Model/enum change:** `OVERLAY_LINE_OPS` extended `("add","remove")` → `("add","remove","edit")`; `ScenarioLineEdit.op` now `add | remove | edit`. `edit` mutates an existing external-cost line's metadata (vendor / cost-type grouping = `sub_category`+`cost_type_id` / description / capex_opex); resolution (`_apply_overlay`) patches the resolved line's `vendor`+`sub_category` in place (€ untouched); editing a still-pending added line patches its `add` row instead of minting an `edit` row (one row per `line_key`). Docs synced same commit: `docs/data-model.md` (model section + Constants & Enums appendix), README API table. **Backend:** 4 endpoints under the T2 banner in `routers/scenarios_project_scope.py` — `GET/POST …/projects/{pid}/external-costs`, `PUT/DELETE …/external-costs/{line_key}` (controller/executive + owner gated, CC-owner excluded = S4; reuses `_recalc_after_overlay_write`/`_grid_to_response`/`_line_kind_for_key`/`_require_project`). GET returns anchor+overlay-resolved line items (vendor, cost-type id+name, description, capex, resolved total €, origin anchor|added) + the cost-type catalogue. New schemas `ExternalCostLine*` in `schemas/scenarios.py`. **Frontend:** net-new `ExternalCostEditor.tsx` (mounted as one child of `ForecastGridSurface`, `onStructureChange→bumpReconcile`) + `useExternalCostEdits.ts` + pure `externalCostForm.ts`; T2 client methods (`scenariosApi.ts`) + context mutations (`ScenarioContext.tsx`, each MARK_STALE + change-feed like `applyCellOverlay`) under `// --- T2: external costs ---` banners. **Tests:** +18 backend integration (`test_router_scenarios_external_costs.py`), +2 resolution unit (edit-op) , +13 frontend vitest (`externalCostForm.test.ts`); `tsc` clean. **Known limit:** `op='edit'` resolves in the sandbox/list but is not yet applied to live Forecast at promote — widening is Session 4.
  - **T3 — Cost-allocation gap closure (spec §3 cost-allocation extension; the long pole)** (teammate `cost-alloc`). **No model/enum change.** Stage-1 add-destination + Stage-2 BTC editing were already implemented in the lever-12 service/router/sandbox (`apply_distribution_create` / `apply_btc_lines_change`, union-aware `_compute_scenario_effective_cost`); the gaps closed were (a) the FE add-destination affordance, hidden behind `!sandboxMode`, now enabled in the cost-alloc sandbox, and (b) **full §3 WYSIWYG parity** — the distribution editor now shows its own forked/added sandbox edge on reload. **Union-aware cascade (boundary waiver, user-approved):** the shared Charging-module Stage-1 cascade was made union-aware **backward-compatibly by construction**. New optional `anchor_version_id` kwarg on `dag_resolver.compute_effective_cost`, `cascade_query.query_cascade_chain`/`_walk_directed`/`_build_node`; when `None` (every canonical Charging caller) the path is **byte-identical** (proved: charging suites unchanged), when set it walks the union-of-forked-vs-anchor edge set. The union rule is factored into three `distribution_service` helpers (`get_forked_sources`, `union_incoming_edges`, `union_outgoing_edges`) reused by BOTH the editor cascade AND `scenario_lever12._compute_scenario_effective_cost` (impact preview) so they cannot drift. New read-only `scenario_lever12.resolve_sandbox_and_anchor`. **Router:** additive optional `scenario_id` query param on `GET /api/charging/cascade/{entity_id}` and `GET /api/charging/entities/{entity_id}/distribution-summary` (resolves sandbox+anchor → union-aware response; omitted = unchanged). **Frontend:** `EntityDistributionEditor` gains `sandboxScenarioId` — in sandbox mode `fetchData` passes `scenario_id` to both cascade + summary so the editor's edges, EUR amounts and edge-ids are all sandbox-sourced and consistent; `CostAllocationSurface` passes `ctx.scenarioId`; `chargingApi.getCascadeChain`/`getEntityDistributionSummary` accept `scenario_id`. **Tests:** +9 service/helper (`test_scenario_lever12.py` union-aware cascade incl. un-forked-upstream-inflow + canonical-unchanged guard; `test_distribution_service.py` `TestUnionEdgeHelpers`) + 3 router (`test_router_charging_cascade.py` `?scenario_id` cascade + summary) + the earlier S3 guard tests (add-destination propagation, sandbox-only union cycle, mixed Stage-1+Stage-2 MDH-balance/union-aware). **Charging-module regression proof:** before/after **279 passed** across dag_resolver/depth_validation/cascade/candidates/distribution(+router)/allocation-breakdown/lever12 — canonical path unchanged; full suite **2279 passed / 0 failed**. README API tables updated (cascade + distribution-summary `scenario_id`). **Known limits (documented):** the scenario `to_business` overlay is NOT reflected on the Stage-1 summary's `to_business_pct` (lives in the impact preview); the add-target candidate picker becomes sandbox-version-scoped once a Stage-1 edit exists (sparse-graph pre-filter; backend `apply_distribution_create` remains the union-aware authority on cycle/sum, 409-surfaced).
  - **Lead integration — debounced cross-portfolio feedback (§7) + verification.** Wired the §7 debounce in `ScenarioContext`: the project's own financials already move live in the surfaces; the expensive cross-portfolio dimensions (capacity / backlog ranking / investment mix / outsourcing ratio = the impact dashboard) now auto-refresh ~1s after editing pauses, with the manual **Recalculate** preserved as an explicit refresh. A new `mutationSeq` (bumped on every diff-changing reducer transition — both `MARK_STALE` *and* `SET_DETAIL markStale`, the real overlay-write path) resets a debounce timer gated on `stale`; a manual Recalculate (clears `stale`) cancels any pending auto-recompute and a scenario switch never fires a stray one. **Bug caught in visual verification & fixed:** the first cut keyed only on the `MARK_STALE` action, but the project-scope overlay writes refresh detail via `SET_DETAIL markStale:true`, so the debounce never fired on real edits (the reducer unit test passed because it exercised `MARK_STALE` directly); fixed to bump `mutationSeq` wherever `stale` is set true, +2 reducer tests for the real edit path. **Visual verification (Playwright, 1440px, light + dark → `qa/screenshots/s3-*.png`):** forecast-grid surface with all four new sections (role lines add/remove, Edit-plan modal, Tier-3 seniority/sourcing mix with the `Tier 3` badge — present for Anna/Tier-3, external-cost line items add/edit/remove) renders clean in both themes; debounce confirmed live (editing Data Engineer Jul 100→500 auto-fired `POST /recalculate` + `GET /impact?year=2026` ~1s later, no manual click, impact dashboard populated); **cost-alloc §3 WYSIWYG parity confirmed** — added a Stage-1 sandbox destination (Rail IT Service Desk, 5%) to Master Data Hub, saved, reloaded → the new edge **renders in the editor table with its union-aware amount (149.656 €)**, light + dark. Demo DB reset to pristine after testing. **Residual finding for the cost-alloc review (not a blocker):** a 409 from the distribution save (sum-rule / cycle) currently surfaces as a full-page "Couldn't load scenario" error-boundary state rather than an inline validation message — recovers cleanly on reload (nothing persisted); worth converting to an inline error in the dedicated review (was reachable in test by bypassing the UI's own total-guard; confirm whether the cycle-409 path can hit it through normal interaction).
  - **Code-review gate — independent `code-reviewer-fresh` on the full S3 diff (no blockers).** Returned 3 should-fix + 4 nits; user approved addressing all. Fixes (same branch):
    - **SF-1 (security):** the new Charging `?scenario_id` endpoints (`get_cascade_chain`, `get_entity_distribution_summary`) read another scenario's sandbox without a visibility check — added `_assert_scenario_visible` (reuses `_get_scenario_or_404` + `_user_can_view_scenario` from `routers.scenarios`; lazy import, no cycle) on the sandbox branch of both; canonical no-`scenario_id` path unchanged. +visibility tests (non-owner `persona-exec` → 403, owner → 200, canonical open).
    - **SF-2:** `write_plan_edit` now validates `start_month <= end_month` against the counterpart's effective value (overlay else live `Project`) → 422 on an inverted window. +tests.
    - **SF-3:** `role_type_id` (role-line add) and `cost_type_id` (external-cost add + edit) are validated against existing rows → 422 if unknown (mirrors the `capex_opex` guard; `RoleType`/`ExternalCostType` have no `is_active`). +tests; `t1_world` fixture now seeds real `RoleType` rows.
    - **N-4:** role-line DELETE route → `{line_key:path}` (symmetry with external-cost routes). **N-5:** comment pinning the one-row-per-line_key `uq_scenario_line_edit` invariant on the resolution loop. **N-6:** `recalcInFlightRef` guard drops overlapping recalculates (debounce vs manual). **N-7 (model change):** added `uq_scenario_mix_change` UniqueConstraint on (`scenario_id`,`project_id`,`cost_center_id`,`swap_from_role_id`,`swap_to_role_id`) — matches the mix upsert, mirrors the sibling overlay tables; `docs/data-model.md` updated; the single seeded mix row does not violate it. **Reseed required** for an existing demo DB (SQLite can't `ALTER … ADD CONSTRAINT`; `create_all` builds it fresh — tests pick it up automatically). The reviewer independently **verified correct** (no action): the Charging backward-compat byte-identical default path, the union-of-forked-vs-anchor rule (no double-count), the debounce/scenario-switch handling, the project-scope endpoint authorization, and the `OVERLAY_LINE_OPS` "edit" widening.

- [x] **Session 4 — PL authoring & access layer + dynamic present time (living demo)** (agent-team split: lead did the temporal-seam keystone + living-seed solo, then fanned out 4 parallel lanes — L2 access backend, L3 apply-to-forecast widening, L4 frontend, L5 seed/tests/docs — + lead integration, branch `feature/simulator-s4-present-time`, 2026-06-03). **Full suite 2365 passed / 0 failed; `tsc --noEmit` clean.** Closes the S3 known-limit (mix/plan/external-edit overlays now widen through apply-to-forecast with a stale-anchor guard), admits Project Leads as scoped scenario authors with directional publication, and replaces the fixed April-2026 demo date with a runtime-computed present (living seed).
  - **Dynamic present time (lead):** `config.DEMO_DATE` is now **dynamic** — `config.get_current_period()` returns the real current month (`date.today()` → `YYYY-MM`); `DEMO_DATE` is computed once at import as a stable, backward-compatible alias for the ~hundreds of existing reads. The **in-progress current month is locked for forecasting**; `services.calendar` adds `current_period()` and `open_forecast_month()` (= `DEMO_DATE + 1`, the editable boundary: `month >= open_forecast_month()`). New unauthenticated endpoint **`GET /api/config`** (`routers/config.py`) → `{current_period, open_forecast_month, fiscal_year}` so the frontend reads the anchor on bootstrap and the two layers can't disagree. Edit-boundary sites migrated from `month >= DEMO_DATE` to the locked-current-month rule.
  - **Living seed (lead):** the committed canonical `seed.sql` stays anchored at `2026-04` (`seed/date_shift.CANONICAL_BASE`) and is **date-shifted by `DELTA = month_delta(CANONICAL_BASE, current_period)` months at load and on `/admin/reset-demo`** via `seed/date_shift.shift_sql_dates` (wired in `seed/loader.load_seed_sql`); `loader._seed_forecast_versions()` re-anchors `ForecastVersion` rows on `config.DEMO_DATE` with dynamic cycle labels. `shift_sql_dates` is a generation-time transform over quoted date/datetime literals (`'YYYY-MM'`, `'YYYY-MM-DD'` with day-of-month clamp e.g. Jan-31+1→Feb-28, `'YYYY-MM-DD HH:MM:SS'` preserving time); dotted-decimal amounts + prefixed ids never match; `delta == 0` is a byte-exact no-op. So actuals always end at the current month, forecasts always open next month, and every narrative offset is preserved without re-hand-editing the seed.
  - **PL authoring & access layer (`L2`):** `project_lead` admitted as a scenario author across `create_scenario` + the owner-gated lifecycle endpoints in `routers/scenarios.py` (promote stays controller-only — a PL cannot promote). **Per-action scoping (§9):** new `dependencies.assert_pl_may_touch_project(db, user, project_id)` (+ predicate `pl_leads_project`) — a PL may only add actions/overlays targeting a project they lead (else 403); controllers/executives unrestricted. **Directional, non-lateral visibility (§9.1/§9.2):** `_user_can_view_scenario` now branches on role — a PL sees own-authored scenarios plus *published* scenarios whose actions touch a led project (the leadership→PL handoff), never a peer PL's scenario; controllers see all published. PL publish flips `status=published` (flows upward + feeds the handoff) without granting a promote route. +11 tests in `tests/test_scenarios_pl_access.py`.
  - **Apply-to-forecast widening + stale-anchor guard (`L3`):** `services/scenario_apply_forecast.py` — `PL_FORECAST_CARRY_CATEGORIES` widened to carry `external_cost` (and, via the `forecast_grid`-classified `ScenarioLineEdit` overlays, added/removed role + external lines) so the full Session-3 edit surface materialises as provisional cells; portfolio/cross-project diffs stay out. **Stale-anchor guard** reuses `scenario_promote.assert_anchor_is_latest_cycle` (re-raised as `ApplyToForecastError(hint="rebase")`, mapped to 409 by the router); it is a deliberate no-op when zero cycle versions exist (preserves legacy applies) and enforces fully once any cycle exists (the living seed now creates them). +10 tests in `tests/test_apply_forecast_widened.py`.
  - **Frontend (`L4-fe`):** new `ConfigContext` (`useConfig()` → `{currentPeriod, openForecastMonth, fiscalYear}`) fetching `GET /api/config` on bootstrap, mounted beside `RoleProvider`; `lib/yearColumns.ts` consolidated (runtime mirror + `isElapsedMonth`/`isEditableMonth` with the locked-current-month rule) and the **8 hard-coded `2026-04` literals** across the forecast/capacity/portfolio surfaces replaced with the dynamic value. Simulator surfaces: PL admitted as author (`useCanCreateScenario`), PL project-picker scoping (non-led greyed/locked), role-aware Publish (`canPublish` = controller|PL, executive excluded; promote stays controller-only), "Handoff from {author}" labelling, and a stale-anchor **Rebase** guard dialog on the apply 409. `npx tsc --noEmit` clean (no new errors).
  - **Integration (lead):** wired `assert_pl_may_touch_project` into the project-scope overlay-write endpoints (`routers/scenarios_project_scope.py` — cell/line/plan/mix/external writes + reverts now admit `project_lead` and enforce scoping), so a PL can edit their own scenario's grid but gets 403 on a non-led project (+test `test_pl_cell_write_scoped_to_led_project`). Updated three now-obsolete tests asserting the old "PL cannot create/apply" rules (`test_router_scenarios.py`, `test_router_scenarios_b1.py`) to the §9 behaviour (PL can create; PL apply requires a current anchor). Fixed the remaining frontend April-2026 doc strings (`DataModelTab.tsx`, `OverviewTab.tsx`) the literal-sweep didn't cover.
  - **PL-authored seed scenario (`L4`):** added a fourth simulator scenario `scn-pl-predmaint-defer` (id 4) to `seed/generate_seed_v5/config/scenarios.py` + the committed `seed.sql`, exercising the **PL authoring path**. Authored by Priya Sharma (`p-sharma`, persona `persona-pl`) and scoped to one of her owned projects, `proj-predmaint` (Predictive Maintenance PoC). A Tier-1 `delay_project` macro (3 months, "relieve sensor-team clash") paired with a Layer-2 `end_month` `ScenarioPlanEdit` (2027-03 → 2027-06) so the resolution engine reshapes the window. `status`/`visibility` = `private`, `cc_owner_scope_cc_id` NULL; visible to Priya via the owner branch in `_user_can_view_scenario`, not laterally to other PLs (§9.2); states/capacity/promotions left empty (the recompute engine computes the snapshot on open, matching scenarios 1 & 3). Got into `seed.sql` by **hand-inserting the rows** in the scenarios section (anchored 2026-04, shifted at load) rather than full-regen — the runner (`python -m generate_seed_v5.runner`) emits only s01–s20 and would drop the separately-appended s21/s22 blocks; the config edit keeps the generator source-of-truth in sync. Verified end-to-end via the full `seed_database()` pipeline (scenario + action + plan-edit load cleanly, FKs resolve, plan-edit shifts with the delta).
  - **Tests (`L4`):** `tests/test_date_shift.py` (19 — month/date/datetime shapes, day-clamp incl. leap year, non-date-token immunity, zero-delta no-op, `month_delta`); `tests/test_api_config.py` (10 — `GET /api/config` shape + `open == current+1` + FY-of-year, parametrised across year-rollover months; patches the router's bound `get_current_period` to keep it consistent under the autouse `pin_demo_date` fixture); `tests/test_living_seed_boundaries.py` (29 — loads the canonical `seed.sql` shifted by delta 0/+2/+14 into a temp sqlite via `Base.metadata.create_all` + `executescript(shift_sql_dates(...))`, asserts actuals end at the shifted current month with none strictly after, forecasts/baselines extend into the future, and the PL scenario's plan-edit shifts correctly). **All 58 new tests pass.** `conftest.py` autouse `pin_demo_date` pins `DEMO_DATE="2026-04"` so the rest of the suite stays deterministic (open forecast month = `2026-05`).
  - **Docs (`L4`):** CLAUDE.md "Demo date is April 2026" bullet rewritten to the dynamic present-time model (real current month, locked in-progress month, next-month open boundary, `/api/config`, living-seed shift). `docs/data-model.md` gained a "Temporal anchor (dynamic present time / living seed)" cross-cutting pattern + corrected the DistributionVersion resolution-rule note (was "Demo date = April 2026"). README API tables document `GET /api/config` + a dynamic-present-time / PL-scenario feature note. Seed fixture manuals updated where they referenced a fixed April-2026 date.
  - **Code-review fixes (lead, post-`code-reviewer-fresh`):** no blockers found; applied the 2 should-fix + 3 nits. **SF-1** — `canPublish` (`ScenarioContext.tsx`) now includes `cost_center_owner` so CC-owner authors can publish from the workspace header (matches the backend gate). **SF-2** — `apply_to_forecast` builds its PL led-set as the union of `user.project_ids` **and** `Project.pl_person_id`-led projects, matching the authoring boundary (`pl_leads_project`) so a project led only via `pl_person_id` is carried, not skipped (+regression test in `test_apply_forecast_widened.py`). **Nit-3** — `/api/config` derives all three fields from one `get_current_period()` snapshot (no live/pinned mix). **Nit-4** — `yearColumns.ts` fallback period computed from `new Date()` instead of a `2026-06` literal. **Nit-5** — `ConfigContext` logs a `console.warn` on `/api/config` fetch failure. Full suite green; `tsc --noEmit` clean.

- [ ] **Session 5 — Naming cleanup & dead-surface retirement** (3-lane parallel fan-out — `react-specialist` (frontend) ∥ `fastapi-developer` (backend) ∥ `doc-writer` (docs) — + lead freeze-gate, integration & visual verification, branch `feature/simulator-project-scope-s5` off `main` @ `ad13beb`, 2026-06-03). Implements spec §11. **Pure cleanup — no behaviour change, no model change, no reseed.** Backend suite **2367 passed / 0 failed**; frontend `tsc --noEmit` clean (`tsc -b` at the 89-error main baseline, 0 new), vitest **143 passed**. **3 atomic commits** (backend / frontend / docs) + this PROGRESS entry.
  - **Frozen contract (lead gate):** before fan-out, froze the two cross-lane tokens so all lanes wrote identical strings — new module name `scenario_cost_allocation.py` and route prefix `/api/scenarios/{id}/cost-allocation/...` — plus the new symbol names (`COST_ALLOCATION_*`, `CostAllocationError`, ChangeSummaryKind `cost_allocation_*`). File ownership split conflict-free: Lane A = `frontend/src/**`, Lane B = `backend/**` code+tests, Lane C = `docs/** + README + CLAUDE.md + fixtures/{manuals,faq}`.
  - **Lane A — frontend retirement + rename (`react-specialist`).** Deleted the **5 collapsed project-scope surfaces** (`ResourceAssignmentSurface`, `MilestonesSurface`, `VendorContractsSurface`, `SourcingMixSurface`, `CapExOpExSurface`) + their barrel exports + `ScenarioWorkspacePage` switch cases (verified orphaned — `ProjectsSection` only ever routes to `forecast-grid`). **Kept** `hierarchy-reassign`, `pipeline-stage`, `tech-navigator-score`, `running-costs` (no clean grid equivalent; conservative). Retired the **10 project-scope catalogue forms** (`catalogue/projectLevel/*` + barrel; `PROJECT_LEVEL` emptied, the `'projectLevel'` category union member + labels kept for `actionsByCategory` record-init) — `BulkActionsPanel` now shows only portfolio-scope levers (empty group skipped). Frontend **"Lever 12" → "cost allocation"**: `scenariosApi` method names + `/cost-allocation/` URLs, `ChangeSummaryKind` values `cost_allocation_*` + `changeSummary` keys (ephemeral client state, safe), "Cost allocation" labels (`PortfolioSettingsSection`, `CostAllocationSurface`, `routingLabels`, `CostAllocationDimension`), comments, charging-module + docs-hub tab strings. Lead dropped 5 now-unused dead alias methods in `scenariosApi` (one collided with the canonical `costAllocationImpact`) and tidied two stale `projectLevel/` dir-reference comments.
  - **Lane B — backend full rename (`fastapi-developer`).** Atomic single-commit rename (no alias): `services/scenario_lever12.py → scenario_cost_allocation.py` + test file; `LEVER12_ACTION_TYPES → COST_ALLOCATION_ACTION_TYPES`, `LEVER12_CATEGORY → COST_ALLOCATION_CATEGORY` (value stays `"cost_allocation"`), `Lever12Error → CostAllocationError`, `cleanup_lever12_state → cleanup_cost_allocation_state`; **HTTP routes `/lever12/* → /cost-allocation/*`** (6 routes + handler fns); `scenario_impact` params `include_cost_allocation`/`cost_allocation_year`; `scenario_promote` internal fns; all importers + comment-only prose across models/schemas/services/routers + seed-generator comments. **Stored values untouched** (action_type strings, `lever_category="cost_allocation"`, `routing_type="cost_allocation_update"`, `seed.sql`, `goals.json`). Suite **2367 passed**.
  - **Lane C — docs + in-app manuals (`doc-writer`).** Rewrote project-scope sections to the **WYSIWYG model** (direct grid editing + macros; old per-action forms retired) and renamed "Lever 12" → "cost-allocation sandbox" (Stage-1 add-destination, full Stage-2 BTC) in `whatif_simulator.json`, `charging.json`, `faq.json`, `docs/simulator-module-overview.md` (newly committed), `docs/data-model.md` (section titles + `scenario_cost_allocation.py` ref), `docs/workflows/06+07`, `README.md` (simulator section + `/cost-allocation/` routes), `CLAUDE.md` L75. Portfolio scope left untouched. Archive (PROGRESS/SETUP/changelog/guides/followups) untouched.
  - **Verification (lead, Playwright 1440px light + dark → `qa/screenshots/s5-cost-allocation-surface-*.png`):** (1) cost-allocation surface loads under `/surface/cost-allocation`, sidebar label is **"Cost allocation"** (no "(Lever 12)"); a Stage-1 save fired **`PUT /api/scenarios/1/cost-allocation/distributions/2`** with **zero `/lever12/` requests** (the 409 it returned is the known S3 batched-save residual, not an S5 regression). (2) **Bulk Actions panel shows only Portfolio rules / Target setters / Restructuring** — the Project-level group is gone. (3) Portfolio-scope surfaces all present (budget envelope, rate tables, escalation, cost allocation, hierarchy reassign) — unchanged. (4) A retired surface key (`/surface/resource-assignment/…`) renders the default placeholder, no crash. Demo DB reset to pristine. **Note:** the seed's MDH scenario description ("Lever 12 demo") + `lever-12` tag are stored seed literals deliberately left unchanged (no reseed this session).
  - **Awaiting code-review gate** (`code-reviewer-fresh`) before any PR.

- [ ] **Session 6 — Integration & verification → location-aware rates (feature build)** (lead + agent team `simulator-s6`: lead drove the rate keystone + integration; teammates `frontend`/`seed-story`/`tester` on disjoint territory, branch `feature/simulator-project-scope-s6` off `main` @ `c40193f`, 2026-06-03). Began as the planned final verification of the project-scope redesign; verifying the folded-in rate fix surfaced a structural issue that the user chose to fix properly, so S6 grew into a multi-layer **location-aware rates** feature. **Full backend suite 2385 passed / 0 failed; frontend `tsc -b` at the 89-error main baseline (0 new) + vitest 149 passed; visual reverify light + dark.**
  - **Rate-at-month fix (`d8fac86`, + regression guards `5f30165`).** Closed the carried-forward S2 nit: `effective_hourly_rate` priced at the *latest* RateTable row, not the rate in force at the cell's month. Routed all scenario project-scope call sites through the existing `services.calculations.resolve_hourly_rate` (effective-date lookup); fallback reconciled to `DEFAULT_HOURLY_RATE`. +2 resolution regression tests; tester added `test_scenario_project_scope_rates.py` + routing/mix/display guards.
  - **Why it grew (the finding).** Visual verification of the MDH Staffing Mix scenario showed a **+€22.400 delta for a cost-saving senior→mid rebalance** — wrong-signed. Root cause: internal work is priced **per workforce location** (Munich/Budapest/Pune) in the seed, but the rate table stored only the Munich rate, forecasts persisted no location, and the grid `line_key` had no location segment — so 8 of 34 projects that staff one role at two locations **blended** into one line, and recompute (Munich rate) disagreed with the anchor (stored per-location €). ~679/1059 internal cells were inconsistent. Probed the magnitude on a throwaway DB; confirmed the cause is structural (per-location pricing vs a Munich-only table; `forecasts` carry no location, so a faithful fix means splitting lines by location). User chose the full location-aware build.
  - **Backend engine keystone (`f5788f1`, lead).** Nullable `location_id` on `forecasts`/`baselines`/`actuals` + `rate_table`; `location_id` on `ScenarioLineEdit`; `swap_from_location_id`/`swap_to_location_id` on `ScenarioMixChange` (+ extended `uq_scenario_mix_change`) — all nullable so the pre-regen seed loads unchanged. `line_key` gained a 4th **location** segment (`category|sub|role|location`); `read_anchor_grid` groups by (cat, sub, role, location) so same-role-multi-location staffing stays split; `_apply_mix` matches + swaps by (role, location). `resolve_hourly_rate(role, cc, month, location_id=None)` precedence: **exact location → loc-muc → cc → any → DEFAULT** (keeps the 3 existing callers stable; capacity external-FTE unchanged). Routing write-backs (`_find_forecast_cell`/`write_forecast_cells`/`_write_grid_cells`/`write_grid_replacing_project` incl. its key tuple) thread location; router `_grid_to_response` + `ScenarioGridRow` expose `location_id`/`location_name`; mix + add-line endpoints/schemas carry location (intra-location swap UX).
  - **Frontend (`6fbb2e7`, teammate `frontend`).** `ScenarioGridRow` + `forecastGridAdapter` 4-part row key; `MonthCategoryGrid` renders the per-line location chip via the shared `<LocationLabel kind="workforce"/>`; `RoleLineControls` add-line gains a workforce-location selector; `Tier3MixControl` gains an intra-location selector. External lines unchanged.
  - **Seed regen (`ce5ede4`, teammate `seed-story`).** `s04_roles_rates` emits one rate row per (role, workforce location, **two effective periods**: 2025-01-01 prev ≈95%, 2026-01-01 current) so pre-open months resolve to a real rate (not the €120 fallback); `s13_financials` persists `location_id` on internal forecast/baseline/actuals rows; the MDH `ScenarioMixChange` sets from/to `loc-muc`; the seeded DWH external descope cell-edit `line_key`s updated to the 4-segment form. **`seed.sql` regenerated via the runner** (verified byte-faithful before/after).
  - **Integration fixes (`9c7f00c`, lead) + callers/tests (`734955b`, teammate `tester`).** Integration caught two bugs: `macros._clone_line` dropped `location_id` when cloning lines for the macro pass (so a macro'd project lost location and the mix swap silently no-op'd → the headline guard caught it); and the external-cost list endpoint built a 3-segment anchor key (vendor/description came back null). `forecast_versioning` now derives the person's location (Person→CostCenter) and passes it to `resolve_hourly_rate`; capacity passes `location_id=None` explicitly. Test ripple greened up: 4-segment `line_key` assertions + seeded overlay keys, anchor line counts for the 8 multi-location projects, a multi-location promote case, and a **senior→mid swap NEGATIVE-delta guard**.
  - **Verification.** Reseeded onto the new schema; the MDH grid now splits **Data Engineer Munich (€100) + Data Engineer Budapest (€70)** as separate labelled lines, Developer Pune (€38) + the swap-created Developer Munich (€82), and the senior→mid rebalance nets **−€5.760 (a saving)**, RAG green — the +€22.400 artifact is gone. Light + dark → `qa/screenshots/s6-14/15-*.png`.
  - **Migration note (SQLite, no Alembic).** `create_all` does not add columns to an existing table and `reset_database()` only `DELETE`s — so an existing dev/demo DB must be **deleted (or the affected tables dropped)** so `create_all` rebuilds them with `location_id`, then `/admin/reset-demo` reloads the location-aware seed. Tests are unaffected (fresh in-memory schema per run).
  - **Carried-forward S6-base items confirmed in passing:** lever-12 seed naming rename (`aad314c`) + the rate-at-month docs (`a053482`) landed early in the session. CLAUDE.md model index unchanged (column-only changes, no model added/removed). `docs/data-model.md` + the in-app simulator manual updated for the new columns (docs lane).
  - **Code-review gate — independent `code-reviewer-fresh` on the full S6 diff (no blockers).** Returned 3 should-fix + 3 nits; user approved addressing all. Fixes (same branch). Full suite **2389 passed / 0 failed**; seeded MDH Staffing Mix delta unchanged at **−€5.760** (future-month swap; the historical re-price cancels in the delta).
    - **SF-1 (precedence intent):** `resolve_hourly_rate` keeps the `loc-muc` fallback ABOVE the competence-centre filter — deliberate, because post location-split a role's rows at all locations share one `competence_center_id`, so a cc-only filter would return a *nondeterministic* location's rate; the deterministic Munich fallback is correct. Added an explaining comment + a regression test (`test_calculations.py::test_location_none_with_cc_prefers_munich_not_cc`) pinning the ordering. No behaviour change.
    - **SF-2 (seed rate-at-month — user chose the principled fix):** `s13_financials._rate_at_month(role, loc, month)` prices each internal baseline/forecast/actuals cell at the rate in force at its month (previous ≈95% before the canonical `2026-01` step, current from it) instead of a single current rate, so the seeded historical anchor equals what the rate-at-month engine recomputes (e.g. `forecast_versioning` historical person-breakdown is now anchor-exact). `seed.sql` regenerated via the runner; full suite stayed green (seed-€ assertions pin current/future or aggregate values).
    - **SF-3 (validate location_id):** `add_role_line` + `write_mix_change` now 422 on an unknown `location_id` (mirrors the role-type guard) — otherwise a bogus location inserts a dangling FK (SQLite FK enforcement is off) and silently prices at the Munich fallback. +2 tests. **Deliberate scope boundary:** did NOT enable a global `PRAGMA foreign_keys=ON` — that would change delete/seed-load/soft-delete semantics app-wide (high regression risk); the explicit validation closes the actual silent-mispricing path.
    - **Nits:** **#4** location-less Tier-3 mix on a multi-location role matches the first split line (`resolution._internal_line_for`) — harmless today (seeded mix carries `loc-muc`); a PL-authored location-less mix is a documented follow-up. **#5** the unrated-role fallback is `DEFAULT_HOURLY_RATE` (€120), deliberately unified with the canonical resolver (the old local €85 was retired) — confirming comment added. **#6** `_grid_to_response` memoises the per-line display rate by `(role, location)` (mirrors `forecast_versioning`'s `rate_cache`) instead of re-querying RateTable per line.
  - **Second `code-reviewer-fresh` pass** (on the bigger-than-expected fix-up delta `1fedd95..HEAD`; **no blockers, no should-fix**) — independently re-derived the SF-2 numbers, confirmed `_rate_at_month` matches the seeded rate rows + engine, the canonical `2026-01` boundary survives `date_shift` (cell months + effective dates shift in lockstep), no double-count, SF-3 has no bypass, the SF-1 test pins the ordering. Its 3 nits all fixed: (1) the dead None-hours actuals fallback in `s13_financials` now scales by the period ratio so it stays rate-at-month if ever reached (seed output byte-identical — branch is dead for internal staffing); (2) `write_mix_change` now also validates `swap_from/to_role_id` against `RoleType` (422), parity with the location guard + `add_role_line` (+test); (3) the external emitter carries a comment that externals are intentionally flat (no previous-period rate).
...
- [x] **Project state-model unification + lifecycle gate fixes** (solo, branch `feature/state-model-unification` off `feature/backlog-expansion-lifecycle-fixes`, 2026-06-01). Plan: `~/.claude/plans/let-s-plan-for-all-synthetic-raven.md`. Triggered by a full lifecycle re-walk that found the legacy `Project.status` column drifting from `pipeline_stage` (a project advanced to Active still read `status='draft'` → Workbench sidebar showed "Draft" while the overview showed "Active"; `status` also drove launchpad/portfolio/workbench/capacity behaviour). **Retired `Project.status` entirely in one pass.** Suite green (**2139 tests**).
  - **Two-axis model (WS1):** dropped `Project.status`; added nullable `Project.review_state` (`pending_cc_confirmation` | `pending_approval` | `changes_requested` | NULL) for the intake/submission review sub-state that *cannot* be derived from the pipeline; made `pipeline_stage` **non-null** (`server_default='Proposed'`) as the single lifecycle source of truth. Seed `s06` drops the `status` column and maps `v4_status` → `review_state` only for genuinely mid-review projects. `models/projects.py`, `seed/generate_seed_v5/`, `docs/data-model.md`.
  - **Backend reads/writes (WS2/WS3):** migrated ~32 read sites + 8 write sites. Lifecycle reads → `pipeline_stage` via the stage-group sets (`BACKLOG_STAGES`/`EXECUTION_STAGES`/`TERMINAL_STAGES`, new `REVIEW_STATES` in `services/pipeline.py`); review reads → `review_state`. `intake_workflow.py`, `global_launchpad.py` (~10 sites + launchpad-submit), `capacity.py`, `workbench.py`, `portfolio.py`, `services/{portfolio_service,report_service,module_card_kpis,report_builder_engine,scenario_engine,capacity_projects}.py`. `review_state` is cleared on approval/advance.
  - **Schemas + frontend (WS4):** dropped `status` from `ProjectDefineResponse`/`IntakeProjectResponse`/`ProjectListItem`/`ProjectMetadata`/capacity responses, exposing `pipeline_stage` + `review_state`. Switched every frontend project-status display to `PipelineStageBadge` (+ a review badge): Workbench sidebar (`EntityListPanel` — the contradiction source), `ProjectHeaderTile`/`MetadataBar`, `ForecastTab`, capacity `ProjectHeader`/`ProjectAssignmentPage`/`ProjectSummaryPanel`, admin `LoBsPanel`, `OverviewSection`, and `types/api.ts`/`types/define.ts`. Verified live: the sidebar now shows the real stage (e.g. autobrake = "Under Evaluation" + "CC Review"), not "Draft".
  - **Lifecycle gate fixes (WS5/WS6):** (#1) entering `Active`/`Hyper-maintenance` now requires ≥1 baseline (confirmed resources) via a new stage-keyed `STAGE_ENTRY_REQUIREMENTS` + `validate_stage_entry` enforced in `transition_pipeline` (409 `stage_entry_unmet`, override-reason escape) — closes the "→ Active at DoI 3 skips the DoI-4 gate" bypass; the transition menu now surfaces the error in the override dialog instead of swallowing it. (#3) `PipelineStageBadge` added to the Define header so stage-only moves are visible. (#5) `project_type` added to the DoI 0→1 gate so a project can't enter the ranked backlog without a P-type.
  - **Tests:** migrated ~64 affected files — an AST-scoped codemod rewrote `Project(status=…)` constructors (status= is overloaded across DistributionVersion/BTCProfile/etc., so AST context was required); conftest `create_test_project` gained a legacy-status→two-axis compat shim + `review_state` param; assertion sites and a handful of helpers (`dict(status=…)`, stubs) fixed by hand; obsolete null-stage / lowercase-status-filter tests updated for the new invariants. Added coverage for the stage-entry gate + the 0→1 `project_type` gate.
  - **Verification:** `pytest tests/ -q` → green (2139); frontend `tsc -b` clean for all touched files (pre-existing unrelated errors remain); reseed boots clean with no `status` column. Live: stage-entry gate returns 409 without a baseline / 200 with override; launchpad + capacity review flows still populate (the `review_state` consumers).
  - **Code + visual review (independent agents, no blockers):** `code-reviewer-fresh` on the diff + a Playwright visual QA walking the full lifecycle across all four personas (headline fix confirmed live — no stale "Draft" badges; sidebar/overview agree; all handovers work; dark mode clean). Both converged on one user-visible defect — the stage-entry 409 rendered as raw JSON — now fixed (the override dialog shows "Cannot advance yet — missing: …"). Also applied: capacity `ProjectHeader` → `PipelineStageBadge`, Report Builder D03 → "Pipeline Stage", validator off-path exemption, and label/banner wording. All confirmed.

- [x] **Backlog expansion + lifecycle handover fixes** (solo, branch `feature/backlog-expansion-lifecycle-fixes` off `origin/main` @ `183c1de`, 2026-06-01). Plan: `~/.claude/plans/let-s-plan-for-all-synthetic-raven.md`. Triggered by a live lifecycle walkthrough (create → score → advance) + an independent fresh-context verification that surfaced two confirmed blockers. Three workstreams. **Suite 2139 passed / 0 failed** (+3 new tests).
  - **WS2 — ChargeableEntity on create (root-cause fix):** `POST /api/projects/define` (and the intake path) created only a `Project` row, never a `ChargeableEntity`; the Workbench sidebar is built from ChargeableEntities (`EntityListPanel.buildEntityRows` silently drops projects with no active entity), so in-app-created projects were **invisible in the Workbench** and unreachable for any workbench-hosted action. New `services/chargeable_entity.py` — `generate_project_identifier(db)` (next free `IT0NNNNN` PPM, parses max Project-entity identifier + loops past collisions) and idempotent `ensure_project_chargeable_entity(db, project, *, hierarchy_node_id)` (mirrors the seed's s06 Project-row shape: `id`==project id, fresh identifier, `s_code`/`allocation_key` NULL, `to_business_pct` 0). Wired into `routers/projects_define.py::create_define_project` and `services/intake_workflow.py::create_intake_project`. +3 tests in `test_router_projects_define.py` (entity minted, distinct identifiers, LoB → hierarchy_node). Verified live: a created project now appears in the Workbench (Project count 27→28, projects == Project-CE count).
  - **WS3 — restore reachable pipeline-advance UI (root-cause fix):** the only transition control (`PipelineTransitionMenu`) was orphaned in both hosts — backlog `DetailHeader` (dead behind the `/backlog/:id → /define/:id` redirect) and workbench `MetadataBar` (replaced by the `OverviewTab` tile grid) — so no UI path existed to advance a project's stage/DoI (backend `/pipeline/transition` + `advance_to_doi` were fully functional). (a) **Forward Advance button** added to the `ApprovalMilestonesTab` readiness card — calls `defineApi.updateApprovalMilestones(id, { advance_to_doi: next })`, shown when `gateMet && !readOnly && next_doi != null`, disabled while the AI-Council buffer is dirty; covers create→Approved→Active for controller (any DoI) and the owning PL (≤ DoI 2, server-gated). (b) **Re-homed `PipelineTransitionMenu`** into the Define header (`DefineShell` actions, controller-only) → restores Pause / Cancel / Reactivate / arbitrary stage moves. Verified live: Advance moved `proj-dwh` DoI 1→2 (audit row written); Stage menu paused it (frozen_doi=2) then the menu offered Reactivate. Light + dark screenshots in `qa/screenshots/`.
  - **WS1 — backlog expansion + visible cutoff:** added **16 pre-execution projects** (9 Proposed / 5 Under Evaluation / 2 Approved; 4 are P3 pre-funded) → **22 backlog projects** total, via the three config touch-points (`config/entities.py` PROJECTS, `config/financials.py` TECH_NAV_SCORES, `s12_pipeline.py` PIPELINE_STATE); financials-free by design (no `PROJECT_STAFFING`/`EXTERNALS`). Lowered the envelope: new seeded planning param `ranking_total_available_budget` = €13M (`config/master.py`), so contestable = €11M and the **should-be cutoff now bites at rank 8** (was "All fit" at €50M). Removed the dead `proj-bk01..bk16` promoted-plans path (`backend/seed/promoted_project_plans.py` deleted + both `loader.py` call sites + helper).
  - **WS1 drift fixes (folded in):** aligned the seed generator's `TECH_NAV_DEFAULT_WEIGHTS` to the live service (value 50/40/10, ranking 70/30 — were 40/40/20 + 50/50) so seeded `composite_score` survives a live recompute; seeded `tn_tshirt_xs/s/m/l_max` planning params (200k/500k/1M/2M) to pin live t-shirt sizing to the seed's wider demo bands; scoped validator rules #1 (Summation) and #7 (Baseline Coverage) to skip pre-execution backlog stages (baselines are an execution-time artifact) — validator now 9/10 (only the pre-existing `d.year` Distribution-Sum bug remains).
  - **Verification:** `pytest tests/ -q` → **2139 passed / 0 failed**; frontend `tsc -b` clean for the changed files (pre-existing `SubmissionDiffView.tsx` errors unrelated). Reseed clean (`seed.sql` regenerated, 27 projects / 50 entities); live backlog shows envelope €13M · contestable €11M · should-be cutoff rank 8 · 4 P3 pre-funded. Demo DB reset to pristine after testing.
  - **Code review (`code-reviewer-fresh`, clean context):** no blockers. One should-fix applied — hide the Advance button for `project_lead` when `next_doi > 2` (backend already 403s PL beyond DoI 2) with an accurate "a controller can advance" hint; verified live (Priya on autobrake DoI 2 → button hidden, controller sees it). Two comment nits applied (stale `tech_navigator.py` t-shirt comment; dropped "(v6)" labels + corrected the 11/34 → 27/50 counts in `entities.py`). Reviewer explicitly validated identifier generation, idempotency, the unique constraints, the frontend refresh/guards, and the validator scoping.
  - **Out of scope (flagged, not fixed):** legacy `status` vs `pipeline_stage` badge split in the Workbench sidebar; DoI-overlay transformation-level deep-link points to the Tech Navigator tab (lives on Approval & Milestones); `?tab=approval` slug mismatch; Financials t-shirt preview only updates post-save; validator rule #9 `no such column: d.year`. Resource-request (Capacity) and CR-approval (Portfolio) handovers not walked this session.
...
- [x] **Wave 4 — Selector + Backlog/Change UI wiring** (3-lane agent-sub-agent fan-out + solo closeout, per user choice; branch `feature/viper-w4-selector-ui` off `main` @ `1b3a9bd`, 2026-05-30). Plan: `~/.claude/plans/let-s-go-wiht-option-squishy-snail.md`. Spec §9 / §4.1 / §3; guide "WAVE 4". The wave where the frontend finally consumes Waves 1–3. UI-heavy — visual-verified in light + dark at 1440px. **Backend suite 2115 passed / 0 failed** (was 2100; +15 run-block tests). Frontend changed files typecheck clean (pre-existing 92-error build baseline unchanged).
  - **Prelude (lead — eliminated the only cross-lane file collision):** pre-added both shared `frontend/src/types/api.ts` fields myself before fan-out — `run?: { entity_count; annual_cost_total }` on `PortfolioKPIs` and `change_status?: string | null` on `ProjectTreeNode` — so the three lanes had fully disjoint file ownership.
  - **Lane 1 — Run selector metrics (backend):** new standalone `compute_run_selector_metrics(db)` in `services/portfolio_service.py` aggregating `ChargeableEntity` where `entity_type in (Offering, InternalService)` and `is_active` → `{entity_count, annual_cost_total}` (sum coalesced, rounded). Merged into the `GET /api/portfolio/kpis` response under a `run` key **at the router layer** (`routers/portfolio.py`) — the shared `compute_portfolio_kpis` is untouched, so Launchpad + module-card KPIs stay unscoped (Wave-2 guard). +`test_portfolio_kpis_run_block.py` (15 tests incl. an isolation guard asserting `compute_portfolio_kpis` emits no `run` key).
  - **Lane 2 — selector segmented bar (frontend):** replaced the pill `SubModuleButton` in `modules/portfolio/PortfolioOverview.tsx` with a full-width two-panel segmented bar (§9.2) — `flex-1` equal panels, `gap-3`, active = `border-2 border-primary bg-primary/10`, inactive = thin border + `bg-muted/40` + `hover:bg-accent`; icon-in-coloured-box (Change `ArrowRightLeft` sky, Run `RefreshCw` emerald, both with `dark:` variants), 16px name, 13px muted metric line. KPIs fetched once at the shell via `portfolioApi.getKPIs()`; Change line = `{active_project_count} projects · {formatCurrency(current_forecast)} forecast`, Run line = `{run.entity_count} entities · {formatCurrency(run.annual_cost_total)} annual cost` (reused `lib/formatters.formatCurrency` — European, e.g. €13,2M). `role="tablist"`/`tab`/`aria-selected` preserved; Run stays the single monolithic `RunPortfolioTab` (contextual Run tabs + Cost Distributions deferred to Wave 5 per user decision).
...
- [x] **Wave 5 — Run "Cost Distributions" tab + Run contextual tabs** (FINAL wave; 3-lane agent team `viper-w5` + lead prelude/closeout, per user choice; branch `feature/viper-w5-cost-distributions` off `main` @ `1808558`, 2026-05-30). Plan: `~/.claude/plans/option-a-let-s-make-pure-cookie.md`. Spec §10; guide "WAVE 5". The only genuinely new surface in the cycle — an org roll-up tree (Rollup mode) + a per-entity **vertical** allocation cascade (Cascade mode), plus the Run sub-module's contextual-tab split deferred from Wave 4. **Backend suite 2127 passed / 0 failed** (+12: cost-tree + run external-spend). Frontend `tsc --noEmit` clean; allocation-flow vitest **45/45**. Visual-verified light + dark at 1440px.
  - **Prelude (lead):** pre-added the one cross-lane contract — `RunCostTreeResponse`/`RunCostTreeNode` (+ `RunCostTreeGroupBy`) in `frontend/src/types/runPortfolio.ts` — so Lane A's endpoint and Lane C's consumer agreed on field names before fan-out. Each lane got disjoint file ownership + an explicit-path staging guard (the shared checkout — `isolation:"worktree"` did not isolate in W4).
  - **Lane A — backend Run cost-tree + Run external-spend:** new `services/run_tree.py` `build_run_cost_tree(db, group_by, node=None, year=2026)` — mirrors `portfolio_service.build_portfolio_tree` recursion but sourced from `chargeable_entities.hierarchy_node_id` (Offering/InternalService only, active) aggregating `annual_cost`/`rolled_cost`. **Node-vs-level rule (§10.2):** Program-attached entities roll up into their parent LoB when grouping `by lob`; an entity attached above the grouping level sits as a direct leaf of its ancestor (no orphaning — node sums == grand total). New `GET /api/portfolio/run/cost-tree?group_by=lob|program&node=&year=` (`RunCostTreeResponse` in `schemas/portfolio.py`); Region/Division/Country reuse the existing `/api/charging/rollup` unchanged. Run-scoped External Spend = additive `population=='run'` branch in `report_service._get_scoped_project_ids` (restrict to `Project.run_entity_id IS NOT NULL`) + three new routes `/portfolio/run/external-costs/{vendor-summary,category-analysis,project-vendor-matrix}` reusing the existing `external_cost_aggregation` services unchanged. +`test_run_cost_tree.py` (9) + `test_run_external_spend.py` (3). `build_portfolio_tree` untouched.
  - **Lane B — allocation-flow promotion + vertical orientation:** moved `modules/workbench/allocation-flow/*` → `components/shared/allocation-flow/` (git-mv, history preserved; App.tsx import repointed). Added optional props `orientation?: 'horizontal'|'vertical'` (default horizontal — workbench byte-equivalent), `embedded?: boolean` (suppresses ModuleHeader/Breadcrumb/Back-to-Workbench + page padding for tab embedding; FocalStrip kept), `entityId?: string` (overrides the URL-driven `?entity=` focal selection). Vertical = axis-swap in `layout.ts`/`edgeGeometry.ts`/`EntityNode.tsx`/`ColumnHeaders.tsx`. **Centering fix (closeout):** the To-Business fan was one ultra-wide row inflating canvasWidth ~2× the panel, so centered content sat at viewport-right; now wrapped into a bounded grid (`VERTICAL_BUSINESS_TARGET_WIDTH=960`) — content fills/centres in the panel, clears the legend. vitest 45/45 (added a permanent centering regression test).
  - **Lane C — Run contextual tabs + Cost Distributions tab:** `PortfolioOverview.tsx` Run mount → route-driven `<Tabs>` **Dashboard / External Spend / Cost Distributions** (no CR Approvals); paths `/portfolio/run`, `/portfolio/run/external-spend`, `/portfolio/run/cost-distributions`; key `viper:portfolio:run:tab`. Dashboard = the prior `RunPortfolioTab` unchanged. External Spend = `ExternalSpendTab` given an optional `scope?: 'change'|'run'` prop (default 'change') that swaps to the Run endpoints. New `RunCostDistributionsTab.tsx` — `[Roll-up | Cascade]` switch + `[Group by ▾][Filter ▾]` + Year (2026); Roll-up renders the cost-tree via shared `ExpandableTreeTable` (LoB/Program) or `RunDimensionRollupPanel` (geo), leaf-click → Cascade; `RunCascadePanel.tsx` mounts `<AllocationFlowView orientation="vertical" embedded entityId={selected} />`. Type tidy: `is_change_or_run` → `'Change'|'Run'`, dropped the "Projects (DoI 5)" Run filter option.
...
- [x] **Wave 5 code-review follow-up** (solo, same branch, 2026-05-31). `code-reviewer-fresh` on the Wave-5 diff returned **0 blockers / 2 should-fix / 2 nits**; user chose to fix all four. **Backend suite 2128 passed / 0 failed** (+1 conservation test); `tsc --noEmit` clean; allocation-flow vitest 45/45; re-verified live + visually (light).
  ...
  - **#2 (should-fix) — Year control gated (`RunCostDistributionsTab.tsx`):** `annual_cost` is a flat scalar so Year had no effect on the LoB/Programme tree or Cascade (only the geo dims hit the year-scoped `chargingApi.getRollup`). The Year `<Labelled>` now renders only when `mode === 'rollup' && !hierarchy` — hidden for LoB/Programme + Cascade, present for Region/Division/Country (verified).
...
- [x] **VIPER post-merge fixes — Run rollup geo dimensions + External Spend enrichment** (solo; one branch `fix/run-rollup-geo-and-external-spend` off `main` @ `836c7d6` — PR #116 merge tip; 2026-06-01). Two items from demo review; the Backlog-cutoff item (#4) is **deferred** to a later backlog-expansion session. **Backend suite 2136 passed / 0 failed** (+8 rollup geo/scope tests); `tsc --noEmit` clean on changed files; visual-verified light + dark at 1440px.
  - **#1 (bug) — Run rollup "By region / division / country" resolved server-side (`services/rollup_query.py`, `routers/charging.py`).** Root cause: `_dim_key_label` **stubbed** `region`/`division`/`country` (and `charging_location`/`legal_entity`) to `ent.entity_type` (lines ~216-219), so all three Dashboard panels (and the Cost Distributions geo group-by, which shares `RunDimensionRollupPanel`) showed an identical entity-type breakdown — including a "Project" bucket although the query was also un-scoped. Geo is not an entity attribute (`ChargeableEntity` has no geo FK) but a **Stage-2 BTC distribution** across charging locations. New `_geo_groups()` fans each entity into one bucket per active `BTCProfileLine`, weighted `effective_cost × to_business_pct/100 × line.pct/100` (mirrors `query_entity_allocation_breakdown` / the Charging module's `useChargingRollupData`), bucketed by the location's region/country/division/location/representative-LE; null → `(Unassigned)`. Added a `change_or_run` scope param to `query_rollup` + the `/api/charging/rollup` endpoint (`'run'` → `entity_type IN (Offering, InternalService)`, `'change'` → Project) since `is_change_or_run` is a computed property. Frontend: `endpoints.ts:getRollup` gains `change_or_run`; `RunDimensionRollupPanel` passes `change_or_run='run'`, drops the dead `void entityIds` no-op (+ removed the now-unused `entityIds` plumbing in `RunPortfolioTab`), and the footer copy corrected to "Cost released to KB business via BTC per [F-S2-01..08]". +8 tests in `test_rollup_query.py` (`TestQueryRollupGeo`, `TestQueryRollupChangeOrRunScope`). **Live (2026):** region EMEA €9,4M / APAC €1,1M / Americas €1,1M; division 4 buckets; country DE/FR/UK/US/IT/IN — distinct, identical grand totals across dims, no Project leakage.
  - **#2 (demo data) — Run External Spend enriched (`seed/seed.sql`).** Only `proj-iam-run` was `run_entity_id`-linked (4 vendors); `proj-cloud3-run` already carried external lines (AWS) but `run_entity_id` was NULL so it was invisible. Wired `proj-cloud3-run` → `svc-infra-platform` (Infrastructure Platform Service — it already distributes 50% there; terminal→terminal stage change, **no backlog/cutoff impact**, keeping #4 decoupled) + added 2026 vendor variety (Microsoft Azure, Datadog, Snowflake on cloud3; Okta on iam-run; actuals Q1 + forecast Apr–Dec, with POs on committed rows). Tab now reads **2 projects / 9 vendors** (Total forecast €1,8M, Actuals YTD €774K). Reseeded (`reset-demo`, clean boot).
  - **Verification:** `pytest tests/ -q` green; live `/api/charging/rollup` (region/division/country, `change_or_run=run`) + `/api/portfolio/run/external-costs/vendor-summary` confirmed; screenshots `qa/screenshots/run-dashboard-geo-{light,dark}.png`, `run-cost-distributions-region-dark.png`, `run-external-spend-dark.png`. Console clean.
  - **Deferred — #4 backlog cutoff:** `ranking_total_available_budget` is unseeded → €50M default ≫ ~€3.5M backlog demand, so the should-be/reality cutoff stays null. Investigated, not changed here — to be tuned alongside expanding the seeded backlog projects (lever: seed a smaller envelope as a `planning_parameters` row).
...
- [x] **Completed the app-wide rename to VIPER** — lifted the §12.1 "dev-facing, out of scope" exclusion and swept every remaining legacy-brand reference across the whole repo (repo docs, code comments/docstrings, seed-generator headers, in-app Doc-Hub + manual content, test names/assertions). Net: `rg -i` for the old token returns zero outside orphaned `.claude/worktrees` checkouts. **Backend suite 2128 passed / 0 failed; `tsc --noEmit` clean on changed files.**
  ...
  - **Independent of PR #117** (rollup geo + external-spend) — merged separately; the only shared file was this PROGRESS.md.
