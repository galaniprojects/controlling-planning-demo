...
**Stream 3 — Backlog cutoff bands now demo-visible (4 → 20 DoI 0–2 projects).** With only 4 backlog (DoI 0–2) projects in the seed, the cumulative-budget walk in `services/ranking.py::compute_ranked_backlog` never crossed the contestable envelope, so `should_be_cutoff_rank` and `reality_cutoff_rank` came back NULL and the `<CutoffBand>` rows in `RankedListTable.tsx` never rendered. Added 16 new backlog projects (`proj-bk01` … `proj-bk16`) to `backend/seed/seed.sql` across five blocks (project rows, chargeable_entities, project_grouping_assignments, Tech Navigator scoring updates, pipeline_stage + DoI updates), with composite scores spread 1.8–4.6 and budgets 95K–2.3M EUR. Also seeded a new `ranking_total_available_budget` PlanningParameter at `5,000,000` EUR (default in code is 50M — too high for demo scale) so the contestable envelope (5M − 1.14M Type 3 pre-funded = 3.86M) crosses the cumulative budget walk mid-list. Result: `should_be_cutoff_rank=4`, `reality_cutoff_rank=16`, misalignment zone spans 13 ranks. The ranked list now has 23 items (20 DoI 0–2 + 3 of the existing Active/Approved DoI 3 projects; `proj-railsafety` and `proj-bk11` move to the Pre-funded Type-3 section). Verified visually at `/backlog`.
