...
- [x] **FD-5 — Dashboard triple-display** (solo session, branch `feature/fd5-dashboard-triple-display`, 2026-05-21). **Final cluster — closes the Charging & Allocations / UM rework round (6/6).** Realises spec §7, tag `[F-DSH-01]`.
  ...
  - **Open items / carry-forward:**
    - **N+1 in `list_btc_profiles`** (review should-fix #1): `_serialize_btc_profile` calls `get_frozen_um_values` (a full `UMVersion` scan) per profile, plus a per-profile `UserMeasurement` query and a lazy `profile.entity` load. Negligible at demo scale (≤13 profiles, 3 UM versions) but a genuine N+1. Proper fix: hoist the `UMVersion` fetch in the list endpoint into an `{activated_at: version}` map, batch the cell query, and `joinedload(BTCProfile.entity)`.
    - UM matrix viewer allocation-key display (spec §3's secondary surface) deferred — needs a modeled `s_code`↔`ChargeableEntity` link or a profile-mediated heuristic; small follow-up if PM wants it.
    - SAP audit `category="export"` (carried from FD-4) still open — add when the audit-log UI grows an export filter chip.

**Charging & Allocations / UM rework round COMPLETE** — six clusters FD-1..FD-6 across PRs #103 (FD-1), #104 (FD-3), #105 (FD-2+FD-4+FD-6), and FD-5 (branch `feature/fd5-dashboard-triple-display`, PR pending). Spec `guides/CRETA_Charging_Allocations_UM_Change_Spec.md` fully realised.
