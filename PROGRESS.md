...
- [x] **Session 1 — Frontend stabilization & apply gate** (agent team `sim-e2e-s1`, 4 teammates with disjoint file ownership + lead integration/visual pass, branch `fix/sim-e2e-s1-stabilization` off `main`, 2026-06-04). Implements spec §5 (apply gate), §7 (recalc lifecycle), §8 (Tier-3 nav), §9 (graceful Stage-2 BTC), §10 (display/formatting). **Frontend vitest 165/165; backend pytest 2370 passed; `tsc --noEmit` clean for all touched files** (pre-existing ~89-error `tsc -b` baseline in unrelated modules unchanged). Visual-verified light + dark @1440px → `qa/screenshots/sim-e2e-s1/`.
  ...
  - **Code-review gate — independent `code-reviewer-fresh` (no blockers; 4 should-fix, all addressed by the lead):** **#2** extracted the shared `parseServerTimestamp` and reused it at both promote-timestamp sites (`PromoteReviewPage` + `PromoteAuditDrawer`) — same UTC-skew class as F5; **#3** reverted `WideSlideOver`'s `return null` (which killed the slide-out animation) to the mounted-but-stripped approach above; **#4** `renderSurface` now passes the `:entityId` segment as `PeopleMasterSurface`'s `initialActionId` so the 4 restructuring shortcuts pre-open their form (not the generic tile list); **#1 (F2) — DEFERRED:** the reviewer flagged the live-local location qualifier as inert. Investigation confirmed the simulator scenario grid carries **no** workforce-location dimension (`Forecast` has no location/CC field; rows group by `(category, sub_category, role_type_id)`; rate resolved by role only) — making real F2 a feature that ports S6's location/competence-centre split into the grid's data path + round-tripped line key + rate resolution + seed (+ reseed), out of S1 scope. Per user decision the inert FE plumbing was **reverted** (4 files restored to `main`).
  - **Deferred / future work:** real **F2** (location-qualified same-role lines in the live-local Internal Resources summary) needs the S6 location dimension threaded through the simulator scenario grid + a reseed — scope it as its own effort, not a frontend nit. Sessions 2 (promote→CR / apply→draft-CR shared capability) and 3 (per-project anchor schema, seed anchors, rebase picker) remain.
