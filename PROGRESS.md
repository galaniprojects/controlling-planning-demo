...
- [x] **Wave 1 — Foundation: lifecycle model, classification, FY helper, rename** (agent team `viper-w1-foundation`, 4 teammates in parallel, single branch `feature/viper-w1-foundation` off `main` @ `26ee420`, 2026-05-29). Plan: `~/.claude/plans/valiant-rolling-sloth.md`. Backend foundation + isolated rename track. **Suite 2062 passed / 0 failed** (was 2053+10-fail mid-integration). Branch tip `aaa6085`.
  ...
  - **Deferred / carry-forward (documented, not silently dropped):**
    ...
    - **Rename tail (low priority, spec §12.1 "opportunistic, not strictly user-facing"):** scattered `creta:*` / `creta_*` localStorage keys + `branding.localStoragePrefix: "creta"`; one user-facing hardcoded CSV filename `creta-sap-export-*.csv` in the charging BTC export (outside the centralized `csvExportPrefix` mechanism). Repo `README.md` / `CLAUDE.md` top-level "CRETA" branding left as dev-facing per §12.1 (internal-identifier scope) — revisit if a full rename is wanted.
