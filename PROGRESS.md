...
## PostgreSQL Migration + Docker + Alembic (2026-06-08)

Branch `feature/postgres-migration` off `main`. Prepares the app for on-prem hosting: migrate from the embedded SQLite file to a **persistent PostgreSQL** database, containerize the full stack, and introduce **Alembic** schema management. The app was already ORM-first (SQLAlchemy 2.0), so the application layer was portable — SQLite assumptions were contained to a handful of spots. Phase intent: a permanently-hosted **playground** now (data persists between restarts; manual reset kept open per user decision), real planning tool later. **Repo split (FE/BE) intentionally out of scope.** Full backend suite **2448 passed** (SQLite); full stack verified live on PostgreSQL via Docker.

- **Env-driven connection** (`config.py`, `database.py`): `DATABASE_URL` from env, **SQLite fallback** retained for no-Docker local dev + tests. `connect_args={"check_same_thread"}` now conditional (SQLite only — psycopg rejects it). Driver: **psycopg 3** (`postgresql+psycopg://`).
- **Reset re-anchors the living-demo date** (user request): `config.reanchor_demo_date()` re-pins the process-wide `DEMO_DATE` to the real current month (mirrors the `conftest.py::pin_demo_date` `sys.modules` walk), called first in `reset_database()`. On a long-running server, `POST /admin/reset-demo` now refreshes "today" without a process restart. Reset is **data-only** (schema is Alembic-managed); endpoint left open/unauthenticated per the playground decision (flagged for hardening at the real-tool transition).
- **Dialect-agnostic seed loader** (`seed/loader.py`): dropped the raw `sqlite3` module / `get_db_path()` / `sqlite_master` / `PRAGMA` usage — all DB access now goes through the shared `engine`/`SessionLocal`. `load_seed_sql` executes via `engine.raw_connection()` (SQLite `executescript` / psycopg `execute`); `is_db_seeded` uses the SQLAlchemy inspector; the three seed helpers reuse `SessionLocal`; `reset_database` wipes via `TRUNCATE ... CASCADE` (PG) / reversed `sorted_tables` delete (SQLite).
- **Seed boolean portability** (`seed/_dialect.py`, NEW): PostgreSQL doesn't coerce integer `0`/`1` into `BOOLEAN`. A metadata-driven, comment-aware converter rewrites boolean-column `0/1`→`TRUE/FALSE` in-memory at load (INSERT tuples **and** UPDATE SET/WHERE), applied after the date-shift — same "transform canonical SQL at load" pattern as `date_shift`. (~3.5k conversions, idempotent, `TRUE/FALSE` valid on SQLite too.)
- **FK ordering on PG**: `seed.sql` carries forward FK references (SQLite tolerates — FK enforcement off by default); `load_seed_sql` sets `session_replication_role = replica` for the PG bulk load (reverted before the pooled connection is released). *Requires the connecting role to be superuser — documented in SETUP.md for locked-down on-prem.*
- **Model `server_default` normalization** (schema change → doc trifecta): 6 Boolean columns `server_default="0"` → `text("false")` (`models/scenarios.py` ×2, `projects.py` ×3, `financial.py` ×1); `CapacityActionLog.timestamp` `"CURRENT_TIMESTAMP"` (string literal — would fail PG `CREATE TABLE`) → `text("CURRENT_TIMESTAMP")` (SQL function). Latent dangling FK fixed: `_seed_forecast_versions` used `created_by_id="p-controller"` (no such person); corrected to the real controller `p-meier` (SQLite ignored the orphan FK; PG rejected it).
- **AI Report read-only sandbox** (`services/ai_report_service.py`): engine now built from `DATABASE_URL`; read-only enforced per dialect — SQLite `PRAGMA query_only`, PostgreSQL `SET SESSION default_transaction_read_only = on`; `BLOCKED_KEYWORDS` denylist kept as defense-in-depth.
- **Alembic** (`backend/alembic/`, `alembic.ini`): baseline migration autogenerated from the current 71-table metadata (verified: applies clean to an empty DB; a second autogenerate is a no-op = perfect parity). `env.py` reads `config.DATABASE_URL`, `target_metadata = Base.metadata`, `render_as_batch` for SQLite. `main.py` startup runs `database.run_migrations()` (replaces `create_all`) — idempotent: empty DB → baseline creates schema; pre-existing-schema-without-`alembic_version` → stamp; else upgrade. `conftest.py` test engine keeps `create_all` (fast in-memory SQLite).
- **Docker** (`docker-compose.yml`, `backend/Dockerfile`, `frontend/Dockerfile` + `nginx.conf`, `.dockerignore`s, `.env.example`): three services — `db` (postgres:16 + named volume `pgdata` + healthcheck), `backend` (entrypoint `alembic upgrade head && uvicorn`), `frontend` (vite build → nginx, proxies `/api` → backend). Frontend image bundles via `npx vite build` (skips the known pre-existing ~90-error `tsc -b` baseline). Three run modes: full Docker / hybrid (container DB + native backend) / pure SQLite.
- **Verification:** backend suite **2448 passed** on SQLite. Live on PostgreSQL via `docker compose up`: baseline migration applied, full seed clean (22 forecast versions, 6 anchors, 0 FK/boolean errors), `/api/config` + `/api/projects` correct (booleans render as real JSON `false`), **persistence** confirmed (27 projects survive `restart`; "already seeded" on reboot), **reset** re-anchors + TRUNCATE-reseeds, frontend renders end-to-end through nginx→backend→PG (`qa/screenshots/postgres-migration-launchpad.png`, 0 console errors).

### Code-review follow-up (independent `code-reviewer-fresh`, no blockers — all 3 should-fix + 4 nits actioned)
- **SF1 — non-superuser-safe seed (replaces `session_replication_role`):** new PG-only migration `d1f2e3a4b5c6_deferrable_fks` makes every FK `DEFERRABLE INITIALLY IMMEDIATE` (runtime FK behaviour unchanged); `load_seed_sql` now issues `SET CONSTRAINTS ALL DEFERRED` so the bulk load's FK checks run at COMMIT. Needs only table ownership, **not superuser** — proven: a `NOSUPERUSER` role gets `permission denied` for `session_replication_role` but `SET CONSTRAINTS ALL DEFERRED` succeeds. SETUP.md / data-model.md notes updated.
- **SF2 — dialect-aware AI Report prompt** (`services/ai_report_service.py`): module-level `_DB_DIALECT` derived from `DATABASE_URL` injected into `SYSTEM_PROMPT` (`{db_dialect}` placeholder + "use {dialect}-compatible SQL") and the two `TOOLS` descriptions; dropped the stale hardcoded "(April 2026)" next to the dynamic `{demo_date}`. Verified the container reports `PostgreSQL`.
- **SF3 — unit tests for the new code** (+14 tests, suite now **2462 passed**): `tests/test_dialect.py` (INSERT/UPDATE boolean conversion, non-boolean `0/1` untouched, idempotency, comment-with-apostrophe desync regression, `archived` vs `archived_at` substring safety, string-embedded `--` preserved), `tests/test_reanchor.py` (re-pin + propagate + logged-failure path), `tests/test_run_migrations.py` (empty→upgrade / pre-existing→stamp / at-head→no-op).
- **N4** — Dockerfile CMD no longer runs `alembic upgrade head`; the app's startup `run_migrations()` is the single source (also covers native runs). **N5** — comment in `load_seed_sql` noting comment-stripping affects both engines. **N6** — read-only denylist extended with `COPY|TRUNCATE|GRANT|REVOKE|CALL|DO`. **N7** — `reanchor_demo_date` logs a warning on per-module propagation failure instead of silently swallowing.
- **Re-verification:** full suite **2462 passed**; clean `docker compose down -v && up` on PostgreSQL applies both migrations and seeds via the deferrable-FK path (no `session_replication_role`), reset re-anchors + reseeds (27 projects), prompt dialect = PostgreSQL.

### Second code-review follow-up (independent `code-reviewer-fresh` #2, no blockers — 4 should-fix + nits actioned)
- **S3 — AI sandbox multi-statement hardening** (`services/ai_report_service.py`): `validate_sql` now rejects any statement-terminating `;` outside string literals (quote-aware `_has_inner_semicolon`, so `SELECT 'a;b'` still passes) — closes the `SELECT …; SET SESSION default_transaction_read_only = off; …` vector; `SET` and `MERGE` added to `BLOCKED_KEYWORDS`.
- **S2 — `execute_sql` truncation off-by-one** (pre-existing): now `fetchmany(ROW_LIMIT + 1)` and `truncated = len > ROW_LIMIT`, so a result of exactly `ROW_LIMIT` rows reports `truncated=False`.
- **S4 — Docker first-boot ordering** (`docker-compose.yml`): `backend` gains a `/health` healthcheck (python-urllib probe — no curl in the slim image; `/health` only answers after startup migrations+seed); `frontend` now `depends_on: backend: condition: service_healthy`, so nginx no longer 502s while the backend seeds.
- **S1 — SQLite reset identity parity** (`seed/loader.py`): the SQLite wipe also clears `sqlite_sequence` (guarded by existence) to match PG `RESTART IDENTITY`.
- **Tests** (+10, suite now **2472 passed**): `tests/test_ai_report_sql.py` — `validate_sql` multi-statement/denylist matrix (incl. quoted-`;` allowed) and `execute_sql` truncation boundary.
- **Nits:** N1 comment (`reanchor_demo_date` refreshes only direct `DEMO_DATE` aliases, not import-time-derived values); N3 comment (`run_migrations` `projects` sentinel); N2 (frontend `.dockerignore` excludes `node_modules`) and N4 already satisfied.
