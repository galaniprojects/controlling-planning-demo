...
- [x] **Session 2 — Promote→draft-CR (F8) + Apply→draft-CR keystone** (agent team `sim-e2e-s2`, lead-solo keystone → 4 parallel lanes with disjoint file ownership + lead integration, branch `fix/sim-e2e-s2-promote-apply-cr` off merged `main` (#127), 2026-06-04). Implements spec §2 (shared CR factory), §3 (promote→CR / F8), §4 (apply→draft-CR). **Full backend pytest green (2400+); frontend vitest 76/76 (simulator), `tsc --noEmit` 0 errors.**
  ...
  - **Remaining future work:** Session 3 (per-project anchor schema, seed anchors, rebase picker — Issue B / F10 / F11) remains.
