...
The monthly forecast cycle is the heartbeat of the VIPER demo: every active project's PL submits a fresh 12-month + quarterly outlook each cycle, the controller reviews + approves, the system snapshots the result as an immutable `ForecastVersion`, and the cycle closes.
