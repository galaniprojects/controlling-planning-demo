...
- **Deactivate (not delete)**: Editing an existing row offers a **Deactivate** action (soft delete). Per VIPER's non-negotiable rule, deactivated rows are hidden from active views but preserved in the DB so historical allocations stay valid.
