...
def generate() -> str:
    ...
    lines.append("")
    lines.append(
        "INSERT INTO chargeable_entities (id, entity_type, identifier, s_code, "
        "name, description, hierarchy_node_id, responsible_person_id, "
        "allocation_key, to_business_pct, project_id, termination_month, "
        "annual_cost, is_active, created_at, modified_at) VALUES"
    )
    rows = []
    for ent in all_chargeable_entities():
        rows.append(
            f"({sql_str(ent['id'])}, {sql_str(ent['entity_type'])}, "
            f"{sql_str(ent['identifier'])}, {sql_str(ent.get('s_code'))}, "
            f"{sql_str(ent['name'])}, NULL, "
