...
# ---------------------------------------------------------------------------
# v5 Session E2 — Role-personalised launchpad tiles per [E-06d]–[E-06j]
# ---------------------------------------------------------------------------


def _format_currency(value: float) -> str:
    """European-style currency for tile display: dot thousands + comma decimals.
...
def _build_tiles_for_pl(db: Session, user: CurrentUser) -> list[TilePayload]:
    ...
    project_ids = [p.id for p in pl_projects]
    project_count = len(pl_projects)
    active_count = sum(1 for p in pl_projects if p.status == "active")

    # My Budget — total budget across own projects
    total_budget = sum(float(p.total_budget or 0) for p in pl_projects)

    # YTD actuals across own projects (year matches DEMO_DATE)
    year_prefix = DEMO_DATE.split("-")[0]
    ytd_actuals = 0.0
    if project_ids:
        from models.financial import Actuals
        ytd_actuals = float(
            db.query(func.coalesce(func.sum(Actuals.amount_eur), 0))
            .filter(
                Actuals.project_id.in_(project_ids),
                func.substr(Actuals.month, 1, 4) == year_prefix,
                func.substr(Actuals.month, 1, 7) <= DEMO_DATE,
            )
            .scalar()
        )

    # My Progress — average progress_pct across active projects
    progress_values = [
        float(p.progress_pct) for p in pl_projects
        if p.progress_pct is not None and p.status == "active"
    ]
    avg_progress = (
    ...
    # Pending forecasts — projects whose last_forecast_submitted_month < DEMO_DATE
    pending_forecasts = sum(
        1 for p in pl_projects
        if p.status == "active" and (
            p.last_forecast_submitted_month is None
            or p.last_forecast_submitted_month < DEMO_DATE
        )
    )

    # Recent CRs against own projects — sent_back / approved / rejected
    recent_crs = 0
    if project_ids:
        recent_crs = (
            ...
            .filter(
                ...
                ChangeRequest.status.in_([
                    "approved", "rejected", "sent_back_by_cc",
                    "sent_back_by_controller", "sent_back",
                ]),
            )
            .scalar()
        ) or 0

    # Published scenarios available to PLs
    scenario_count = (
    ...
    # Resource availability proxy — count of pending resource requests in own projects
    pending_resource_reqs = 0
    if project_ids:
        pending_resource_reqs = (
            db.query(func.count(ResourceRequest.id))
            .filter(
                ResourceRequest.project_id.in_(project_ids),
    ...
    return [
        TilePayload(
            tile_id="pl-my-projects",
            title="My Projects",
            primary_metric=f"{project_count} project{'s' if project_count != 1 else ''}",
            secondary_metric=f"{active_count} active",
            link_module="workbench",
        ),
        TilePayload(
            tile_id="pl-my-budget",
            title="My Budget",
            primary_metric=_format_currency(total_budget),
            secondary_metric=f"{_format_currency(ytd_actuals)} YTD actuals",
            link_module="workbench",
            tone="neutral",
        ),
        TilePayload(
            tile_id="pl-my-progress",
            title="My Progress",
            primary_metric=f"{avg_progress}%",
            secondary_metric=f"average across {len(progress_values)} active",
            link_module="workbench",
        ),
        TilePayload(
            tile_id="pl-my-forecast",
            title="My Forecast",
            primary_metric=(
                f"{pending_forecasts} pending"
                if pending_forecasts > 0 else "All up to date"
            ),
            secondary_metric=f"cycle {DEMO_DATE}",
            link_module="workbench",
            link_tab="forecast",
            tone="warning" if pending_forecasts > 0 else "positive",
        ),
        TilePayload(
            tile_id="pl-recent-changes",
            title="Recent Changes",
            primary_metric=f"{recent_crs} CR{'s' if recent_crs != 1 else ''}",
            secondary_metric="approved or returned",
            link_module="workbench",
            link_tab="history",
        ),
        TilePayload(
            tile_id="pl-scenario-explorer",
            title="Scenario Explorer",
            primary_metric=f"{scenario_count} published",
            secondary_metric="explore impact on your projects",
            link_module="simulator",
        ),
        TilePayload(
            tile_id="pl-resource-availability",
            title="Resource Availability",
            primary_metric=(
                f"{pending_resource_reqs} pending"
                if pending_resource_reqs > 0
                else "No open requests"
            ),
            secondary_metric="role × location aggregates",
            link_module="capacity",
            link_tab="availability",
            tone="warning" if pending_resource_reqs > 0 else "neutral",
        ),
    ]


def _build_tiles_for_controller(db: Session, user: CurrentUser) -> list[TilePayload]:
    ...
    active_count = kpis.get("active_project_count", 0)
    plan_drift_pct = kpis.get("plan_drift_pct", 0)
    current_forecast = kpis.get("current_forecast", 0)

    # Pipeline — projects in proposed/under-evaluation
    pipeline_count = (
        db.query(func.count(Project.id))
        .filter(
            Project.is_active.is_(True),
    ...
    below_cutoff = (
        db.query(func.count(Project.id))
        .filter(Project.is_active.is_(True), Project.within_cutoff.is_(False))
        .scalar()
    ) or 0

    # Pending CR approvals + intake reviews
    pending_crs = (
        db.query(func.count(ChangeRequest.id))
        .filter(ChangeRequest.status == "pending_controller_approval")
        .scalar()
    ...
    total_pending = pending_crs + pending_intake

    # Forecast cycle progress
    overdue_forecasts = (
        db.query(func.count(Project.id))
        .filter(
            Project.is_active.is_(True),
    ...
    # Capacity — over-allocated CCs
    over_allocated = 0
    try:
        # Best-effort: count CCs with allocations > FTE in current month
    ...
    private_scenarios = (
        db.query(func.count(Scenario.id))
        .filter(Scenario.status == "private")
        .scalar()
    ) or 0

    return [
        TilePayload(
            tile_id="ctrl-portfolio-kpis",
            title="Portfolio KPIs",
            primary_metric=_format_currency(float(current_forecast)),
            secondary_metric=f"{active_count} active · drift {plan_drift_pct:+.1f}%",
            link_module="portfolio",
            tone="warning" if abs(plan_drift_pct) > 5 else "neutral",
        ),
        TilePayload(
            tile_id="ctrl-pipeline",
            title="Pipeline",
            primary_metric=f"{pipeline_count} project{'s' if pipeline_count != 1 else ''}",
            secondary_metric="proposed or under evaluation",
            link_module="backlog",
        ),
        TilePayload(
            tile_id="ctrl-budget-vs-cutoff",
            title="Budget vs Cutoff",
            primary_metric=f"{above_cutoff} above · {below_cutoff} below",
            secondary_metric="ranked by composite score",
            link_module="backlog",
        ),
        TilePayload(
            tile_id="ctrl-reporting",
            title="Reporting",
            primary_metric="5 standard reports",
            secondary_metric="plus AI Report Builder",
            link_module="reporting",
        ),
        TilePayload(
            tile_id="ctrl-forecast-cycle",
            title="Forecast Cycle",
            primary_metric=(
                f"{overdue_forecasts} overdue"
                if overdue_forecasts > 0
                else "All current"
            ),
            secondary_metric=f"cycle {DEMO_DATE}",
            link_module="portfolio",
            tone="alert" if overdue_forecasts > 5 else (
                "warning" if overdue_forecasts > 0 else "positive"
            ),
        ),
        TilePayload(
            tile_id="ctrl-pending-reviews",
            title="Pending Reviews",
            primary_metric=f"{total_pending} item{'s' if total_pending != 1 else ''}",
            secondary_metric=f"{pending_crs} CR · {pending_intake} intake",
            link_module="portfolio",
            link_tab="approvals",
            tone="alert" if total_pending > 5 else (
                "warning" if total_pending > 0 else "neutral"
            ),
        ),
        TilePayload(
            tile_id="ctrl-capacity-overview",
            title="Capacity Overview",
            primary_metric=f"{over_allocated} allocated",
            secondary_metric="org-wide utilisation",
            link_module="capacity",
        ),
        TilePayload(
            tile_id="ctrl-scenario-activity",
            title="Scenario Activity",
            primary_metric=f"{published_scenarios} published",
            secondary_metric=f"{private_scenarios} draft",
            link_module="simulator",
        ),
        TilePayload(
            tile_id="ctrl-admin",
            title="Admin",
            primary_metric="System configuration",
            secondary_metric="rates, hierarchy, audit log",
            link_module="admin",
        ),
    ]


def _build_tiles_for_cc_owner(db: Session, user: CurrentUser) -> list[TilePayload]:
    ...
    if cc_id:
        people = (
            db.query(Person)
            .filter(Person.cost_center_id == cc_id, Person.is_active.is_(True))
            .all()
        )
        ...
        if headcount:
            ...
            utils = [
                round((alloc_map.get(p.id, 0) / FTE_HOURS) * 100, 1)
                for p in people
            ]
            avg_util = round(sum(utils) / len(utils), 1) if utils else 0.0
            over_allocated = sum(1 for u in utils if u > 100)
    ...
    if cc_id:
        open_requests = (
            db.query(func.count(ResourceRequest.id))
            .filter(
                ResourceRequest.cost_center_id == cc_id,
                ResourceRequest.status == "pending",
            )
            .scalar()
        ) or 0
        pending_resource_count = open_requests

    # CC Budget — sum of forecasts for projects in this CC's scope
    cc_project_ids: list[str] = []
    cc_forecast_total = 0.0
    if cc_id:
        cc_person_ids = [
        ...
        if cc_person_ids:
            cc_project_ids = [
                r[0] for r in db.query(func.distinct(Allocation.project_id))
                .filter(Allocation.person_id.in_(cc_person_ids)).all()
            ]
        if cc_project_ids:
            cc_forecast_total = float(
                db.query(func.coalesce(func.sum(Forecast.amount_eur), 0))
                .filter(Forecast.project_id.in_(cc_project_ids))
                .scalar()
            )

    cc_active_projects = 0
    if cc_project_ids:
        cc_active_projects = (
            db.query(func.count(Project.id))
            .filter(
                Project.id.in_(cc_project_ids),
                Project.status == "active",
                Project.is_active.is_(True),
            )
            .scalar()
    ...
    return [
        TilePayload(
            tile_id="cco-team-utilization",
            title="Team Utilization",
            primary_metric=f"{avg_util}%",
            secondary_metric=f"{over_allocated} over-allocated",
            link_module="capacity",
            tone="alert" if over_allocated > 0 else (
                "warning" if avg_util > 85 else "positive"
            ),
        ),
        TilePayload(
            tile_id="cco-open-requests",
            title="Open Requests",
            primary_metric=f"{open_requests} pending",
            secondary_metric="awaiting your confirmation",
            link_module="capacity",
            link_tab="requests",
            tone="alert" if open_requests > 5 else (
                "warning" if open_requests > 0 else "neutral"
            ),
        ),
        TilePayload(
            tile_id="cco-headcount",
            title="Headcount",
            primary_metric=f"{headcount} people",
            secondary_metric="active in your cost center",
            link_module="capacity",
        ),
        TilePayload(
            tile_id="cco-cc-budget",
            title="CC Budget",
            primary_metric=_format_currency(cc_forecast_total),
            secondary_metric=f"{cc_active_projects} active projects",
            link_module="capacity",
        ),
        TilePayload(
            tile_id="cco-portfolio",
            title="Portfolio",
            primary_metric=f"{portfolio_total_projects} active",
            secondary_metric="org-wide overview",
            link_module="portfolio",
        ),
        TilePayload(
            tile_id="cco-my-cc-projects",
            title="My CC's Projects",
            primary_metric=f"{cc_active_projects} project{'s' if cc_active_projects != 1 else ''}",
            secondary_metric="with allocations from your CC",
            link_module="portfolio",
        ),
        TilePayload(
            tile_id="cco-published-scenarios",
            title="Published Scenarios",
            primary_metric=f"{published_scenarios}",
            secondary_metric="capacity impact on your CC",
            link_module="simulator",
        ),
        TilePayload(
            tile_id="cco-cc-simulator",
            title="CC Simulator",
            primary_metric="What-if planning",
            secondary_metric="model resource shifts",
            link_module="simulator",
        ),
    ]


def _build_tiles_for_executive(db: Session, user: CurrentUser) -> list[TilePayload]:
    """7 Executive tiles per [E-06e]: Portfolio KPIs, Investment Mix,
    Pipeline Health, Top Risks, Scenario Activity, Budget Trajectory,
    Backlog."""
    kpis = compute_portfolio_kpis(db)
    active_count = kpis.get("active_project_count", 0)
    current_forecast = kpis.get("current_forecast", 0)
    plan_drift_pct = kpis.get("plan_drift_pct", 0)
    run_pct = kpis.get("run_pct", 50)
    change_pct = kpis.get("change_pct", 50)
    ...
    # Top risks — projects with red RAG status
    red_count = (
        db.query(func.count(Project.id))
        .filter(
            Project.is_active.is_(True), Project.rag_status == "red",
            Project.status == "active",
        )
        .scalar()
    ) or 0
    amber_count = (
        db.query(func.count(Project.id))
        .filter(
            Project.is_active.is_(True), Project.rag_status == "amber",
            Project.status == "active",
        )
        .scalar()
    ) or 0
    ...
    # Backlog — count above cutoff (pre-funded handled separately on the page)
    return [
        TilePayload(
            tile_id="exec-portfolio-kpis",
            title="Portfolio KPIs",
            primary_metric=_format_currency(float(current_forecast)),
            secondary_metric=f"{active_count} active · drift {plan_drift_pct:+.1f}%",
            link_module="portfolio",
        ...
        TilePayload(
            tile_id="exec-investment-mix",
            title="Investment Mix",
            primary_metric=f"Run {run_pct}% · Change {change_pct}%",
            secondary_metric="DoI 0–4 / 5 split",
            link_module="charging",
        ),
        TilePayload(
            tile_id="exec-pipeline-health",
            title="Pipeline Health",
            primary_metric=f"{pipeline_count} candidate{'s' if pipeline_count != 1 else ''}",
            secondary_metric=f"{above_cutoff} above cutoff",
            link_module="backlog",
        ),
        TilePayload(
            tile_id="exec-top-risks",
            title="Top Risks",
            primary_metric=f"{red_count} red · {amber_count} amber",
            secondary_metric="active projects flagged",
            link_module="portfolio",
            tone="alert" if red_count > 0 else (
                "warning" if amber_count > 3 else "positive"
            ),
        ),
        TilePayload(
            tile_id="exec-scenario-activity",
            title="Scenario Activity",
            primary_metric=f"{published_scenarios} published",
            secondary_metric="ready for executive review",
            link_module="simulator",
        ),
        TilePayload(
            tile_id="exec-budget-trajectory",
            title="Budget Trajectory",
            primary_metric=f"{plan_drift_pct:+.1f}% drift",
            secondary_metric="forecast vs. baseline",
            link_module="reporting",
            tone="warning" if abs(plan_drift_pct) > 5 else "neutral",
        ),
        TilePayload(
            tile_id="exec-backlog",
            title="Backlog",
            primary_metric=f"{above_cutoff} ranked",
            secondary_metric="cube view available",
            link_module="backlog",
        ),
    ]


@router.get("/launchpad/tiles", response_model=TilesResponse)
def get_launchpad_tiles(
    db: Session = Depends(get_db),
    user: CurrentUser = Depends(get_current_user),
):
    """Return role-personalised launchpad tiles per [E-06d]–[E-06j].

    PL = 7 tiles, Controller = 9, CC Owner = 8, Executive = 7.
    Tile shape is identical across roles so the front-end can render them
    with one component.
    """
    if user.role == "project_lead":
        items = _build_tiles_for_pl(db, user)
    elif user.role == "controller":
        items = _build_tiles_for_controller(db, user)
    elif user.role == "cost_center_owner":
        items = _build_tiles_for_cc_owner(db, user)
    elif user.role == "executive":
        items = _build_tiles_for_executive(db, user)
    else:
        items = []

    return TilesResponse(role=user.role, items=items, total=len(items))
