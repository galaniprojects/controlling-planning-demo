...
export function RequestListPanel({
  ...
  if (collapsed) {
    return (
      <div className="flex h-full w-10 flex-col items-center border-r border-border bg-muted/50 pt-3">
        <button
          type="button"
          onClick={onToggleCollapse}
          className="h-7 w-7 rounded border border-border bg-card text-xs text-muted-foreground hover:bg-muted"
        >
          &rsaquo;
        </button>
      </div>
  ...
  return (
    <div className="flex h-full w-[300px] flex-col border-r border-border bg-muted/50">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {requests.length === 0 && (
          <p className="px-3 py-6 text-sm text-muted-foreground text-center">No requests.</p>
        )}
      </div>
    </div>
  );
