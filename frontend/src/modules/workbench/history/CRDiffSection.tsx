...
export function CRDiffSection({ projectId, crId, onActionComplete }: Props) {
  ...
  const handleAccept = async () => {
    setSubmitting(true);
    try {
      await workbenchApi.acceptCRChanges(projectId, crId);
      setResult('Changes accepted. Change request updated.');
      onActionComplete();
    } catch {
  ...
  const handleResubmit = async () => {
    setSubmitting(true);
    try {
      await workbenchApi.resubmitCR(projectId, crId);
      setResult('Change request resubmitted for controller review.');
      onActionComplete();
    } catch {
      setResult('Failed to resubmit. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-4">
      ...
      {!result && (
        <div className="flex gap-3 pt-2">
          ...
          <Button variant="outline" onClick={handleResubmit} disabled={submitting}>
