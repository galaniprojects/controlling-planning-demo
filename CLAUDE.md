...
- `Forecast` — living plan updated via approved CRs (hours/costs per month). v5 Session C1 adds `is_provisional` Boolean (`server_default="0"`) — `True` for cells beyond the granularity boundary (outer zone per `[C-FG-07]`). Manual CR writes clear this flag. v5.1 C-09 adds three nullable per-month columns for the External Costs monthly grid: `po_amount` (`Numeric(14,2)`, monthly committed PO obligation), `accrual_amount` (`Numeric(14,2)`, monthly accrual estimate), `contract_end_month` (`String(7)` `YYYY-MM`, denormalised line-level metadata). All three default to 0/None and only populate on `category='external'` rows.
...
- `Actuals` — read-only historical spend. v5.1 C-09 adds two nullable columns: `po_number` (`String(50)`, mirrors `Forecast.po_number` so `open_po` reconciles per (vendor, po_number) pair) and `invoiced_amount` (`Numeric(14,2)`, subset of `amount_eur` actually invoiced — drives Remaining Not Invoiced KPI).
...
- `ExternalCostDelivery` — v5.1 C-09 row-expansion content for the External Costs monthly grid. One row per scheduled delivery milestone, keyed implicitly by `(project_id, vendor, po_number)`. Carries `milestone_name`, `expected_month` (YYYY-MM), `expected_amount`, optional `delivered_month` (null = not yet delivered). Loaded only on row-expansion path.
- `ExternalCostInvoice` — v5.1 C-09 row-expansion content. One row per invoice received against an external-cost PO line. Carries `invoice_number`, `invoice_date`, `amount`, `status` (`'received'` | `'paid'`). Loaded only on row-expansion path.
- **External cost status vocabulary (v5.1 C-09):** `Forecast.ext_status` / `Baseline.ext_status` / `Actuals.ext_status` use the 6-value lifecycle set: `planned` / `ordered` / `goods_received` / `invoiced` / `accrual` / `open`. Mirrored in `frontend/src/types/api.ts::ExternalCostStatus` and `backend/schemas/external_costs.py::ExternalCostStatus`. The frontend `ExternalCostStatusBadge` component owns the visual mapping (badge colour per status). Wave 5 remap (`accrued`→`accrual`, `committed`→`ordered`, `delivered`→`goods_received`, `requested`→`planned`) was a one-time seed sed; `open` introduced via the same wave's seed top-up to mark partial-PO-fulfillment lines.
...
- `ResourceRequestAssignment` — per-month person assignments. v5.2 W1 relaxed the unique constraint from `(resource_request_id, month)` to `(resource_request_id, month, person_id)` per spec §9.5 so a single (request, month) can be split across multiple people. Renamed to `uq_rra_request_month_person`.
- `CapacityActionLog` — per-action audit trail for the capacity workflow per v5.2 spec §12.10. Distinct from the system-wide `AuditLog` (8 categories) because the dashboard / history page (§12.11–§12.15) and inbox "Recently completed" section (§12.8) need richer per-action structured data than the generic audit log carries. Columns: `id`, `timestamp` (default CURRENT_TIMESTAMP), `action_type` (`confirm | partial_confirm | decline | decline_request | assign_draft | cr_reconfirm` per spec §12.10), `acting_user_id` (FK people.id), `project_id` (FK projects.id), `cost_center_id` (FK cost_centers.id), `summary` (Text, human-readable), `detail_payload` (Text, JSON-serialised per §12.10 schema: `requests_affected`, `assignments`, `cr_id`, `decline_reason`), `cr_id` (FK change_requests.id, non-null only for `cr_reconfirm`). Four indexes: `ix_capacity_action_log_user`, `ix_capacity_action_log_project`, `ix_capacity_action_log_cc`, `ix_capacity_action_log_time`. Writes are wired via `services/capacity_audit.log_capacity_action()` into four mutating handlers in `routers/capacity.py` (project-confirmation/confirm, project-confirmation/decline, requests/assignments PUT, requests/partially-fulfill PUT). Reads gated by `require_role("controller", "executive", "cost_center_owner")` with server-side scope enforcement on CC Owners per spec §12.14.
