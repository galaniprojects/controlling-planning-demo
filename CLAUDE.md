...
- **Dynamic present time (living demo)** — VIPER's "today" is the **real current month**, not a fixed literal. `config.get_current_period()` returns `date.today()` as `YYYY-MM`; `config.DEMO_DATE` pins it once per process at import (stable, backward-compatible alias for the ~hundreds of existing reads — new code prefers `get_current_period()` / `services.calendar.open_forecast_month()`). The **in-progress current month is LOCKED for forecasting** (it carries partial actuals); the **first editable forecast month is the next one** — `services.calendar.open_forecast_month()` = `DEMO_DATE + 1`, and a month is editable iff `month >= open_forecast_month()`. All time-dependent logic (elapsed-month tinting, actuals cutoffs, forecast boundaries, pending-action triggers) derives from this anchor. The frontend reads it on bootstrap via `GET /api/config` → `{current_period, open_forecast_month, fiscal_year}`. **Living seed:** the committed canonical `seed.sql` is anchored at `2026-04` (`seed/date_shift.CANONICAL_BASE`) and **date-shifted forward by `month_delta(CANONICAL_BASE, current_period)` months at load and on `/admin/reset-demo`** (`seed/date_shift.shift_sql_dates` via `seed/loader.py`), so actuals always end at the current month, forecasts always open next month, and every narrative beat keeps its relative offset without re-hand-editing the seed. Tests pin `DEMO_DATE="2026-04"` via the autouse `pin_demo_date` fixture in `conftest.py` (so `open_forecast_month() == "2026-05"`).
